...
try{
 ...
 for(let pass=1;pass<=5;pass++){
  ...
  else{assert.equal(s.status,'COMPLETED');assert.equal(remote(s).state,'AVAILABLE');assert.equal(remote(s).attributes.identityDigest,approval.expectedIdentitySha256);assert.equal(s.entities.find(e=>e.id==='host:controller-local')?.attributes.physicalIdentityDigest,approval.controllerIdentitySha256);}
 ...
 await page.locator('[data-view="estate"]').click();await page.waitForFunction(()=>(window as any).AgentControlEstate.state().active);await page.evaluate(id=>(window as any).AgentControlEstate.select(id),hostId);
