...
try{
 await page.goto(base);await page.locator('#operator-button').click();await page.locator('#operator-token').fill(fs.readFileSync(tokenFile,'utf8').trim());await page.locator('#operator-form button[type="submit"]').click();await page.locator('#operator-dialog').waitFor({state:'hidden'});await page.locator('#poe-pet-state').waitFor();
 ...
 const pet=page.locator('#poe-launcher'),before=await pet.boundingBox();await pet.focus();await page.keyboard.press('ArrowLeft');const after=await pet.boundingBox();assert.ok(after.x<before.x);await page.keyboard.press('Enter');
