...
async function main() {
  ...
  const expectedInitiatingRequest = parent.origin?.request ?? parent.prompt;
  ...
  const evidence = {
    ...
    request: {source: options.ingress === 'openwa' ? 'authenticated enrolled OpenWA sender through SocialVoiceCoordinator' : 'authenticated dashboard POST /api/parcels', exactInitiatingRequest: expectedInitiatingRequest, governedObjective: QUALIFICATION_PROMPT, origin: parent.origin, parentParcelId: parent.id, parentRunIds},
