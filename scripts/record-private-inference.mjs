...
try {
 ...
 if(fs.existsSync(path.join(root,'result.json'))){await page.click('[data-job-platform-tab="runs"]');await page.locator('[data-parameterized-run]').first().click();await page.locator('[data-parameterized-transcript]').click();await page.click('[data-job-platform-tab=runs]');const transcript=page.locator('#job-platform-runs .execution-transcript pre');await transcript.waitFor({state:'visible'});await transcript.scrollIntoViewIfNeeded();await delay(3000);await page.screenshot({path:path.join(root,'transcript-origin.png')});for(let n=0;n<100;n++){const bottom=await transcript.evaluate(e=>{e.scrollTop+=450;return e.scrollTop+e.clientHeight>=e.scrollHeight});await delay(500);if(bottom)break;}await delay(3000);const scroll=await transcript.evaluate(e=>({top:e.scrollTop,height:e.scrollHeight,viewport:e.clientHeight,atBottom:e.scrollTop+e.clientHeight>=e.scrollHeight-2}));fs.writeFileSync(path.join(root,'transcript-scroll.json'),JSON.stringify(scroll));if(!scroll.atBottom)throw new Error('Transcript did not remain at bottom across dashboard refresh');await page.screenshot({path:path.join(root,'transcript-end.png')});}
