...
export async function cliMain(argv=process.argv.slice(2),io={out:console.log,error:console.error},deps={}){
 ...
 try{
  ...
  if(command==='run'){
   ...
   if(item.kind==='core'){if(['name','saved-id','model','context','profile'].some(k=>options[k]!==undefined))throw new CliError(EXIT.USAGE,'This core catalogue job accepts only its declared parameters.');run=await client.post(`/api/jobs/${encoded(id)}/run`,{parameters});}
   else{if(options.profile)throw new CliError(EXIT.USAGE,'Select a report profile after execution with `ac job ID output --profile ...`.');const context=options.context??'STANDARD';if(!['THIN','STANDARD','DEEP'].includes(context))throw new CliError(EXIT.USAGE,'--context must be THIN, STANDARD or DEEP.');const saved=await client.post('/api/saved-jobs',{id:options['saved-id']??`cli-${id.slice(0,28)}-${randomUUID().slice(0,8)}`,name:options.name??item.name,definition:{id,version:item.definition.version,follow:'pinned'},parameters,...(options.model?{routing:{model:options.model,allowFallback:false}}:{}),contextProfile:context,concurrency:'forbid-overlap',enabled:true});try{run=await client.post(`/api/saved-jobs/${encoded(saved.id)}/run`,{});}catch(error){throw new CliError(error.code??EXIT.UNAVAILABLE,`${error.message} Saved Job ${saved.id} was created; inspect it before retrying.`);}}
