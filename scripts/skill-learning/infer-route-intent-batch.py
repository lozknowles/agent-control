...
for item in request['items']:
 ...
 rows.append({'id':item['id'],'output':output,'inputTokens':int(encoded['input_ids'].numel()),'outputTokens':int(generated.shape[1]-encoded['input_ids'].shape[1]),'elapsedMs':round((time.time()-before)*1000)})
print(json.dumps({'loadMs':round((loaded-started)*1000),'elapsedMs':round((time.time()-started)*1000),'rows':rows}))
