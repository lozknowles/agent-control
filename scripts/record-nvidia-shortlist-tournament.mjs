...
try {
  ...
  for(const candidate of candidates){
    ...
    const result=await action.catch(error=>({status:'FAIL',inferenceEndpointStatus:'INDETERMINATE',failure:sanitize(error.message),failureClass:'PROVIDER_ERROR',usage:null,elapsedMs:null}));
  ...
  const source={branch:git(['branch','--show-current']),head:git(['rev-parse','HEAD']),version:initialStatus.version,woprCrewSchema:initialStatus.characterCrew.schema,expectedWoprCheckpoint:process.env.AGENT_CONTROL_EXPECTED_WOPR_CHECKPOINT??null,nvidiaCheckpoint:process.env.AGENT_CONTROL_NVIDIA_CHECKPOINT??null};
