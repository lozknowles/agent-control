...
try {
  ...
  for (const phase of ['TIMEOUT_COMPLETE','UNCERTAINTY_COMPLETE']) { const value = await waitPhase(phase, 20_000); await page.waitForSelector(`[data-run="${value.runId}"]`, {timeout: 10_000}); await page.click(`[data-run="${value.runId}"]`); await delay(2_500); }
