...
async function main() {
  ...
  const nodeExecution = new ResourceCodexNodeExecutionPort(resources, process.env), runtime = buildJobRuntime(config, stateRoot, undefined, planner, registry, nodeExecution);
  ...
  const control = new AgentControlService(state, new PtyRegistry(), undefined, '3.9.0-protected-resource-qualification', () => {}).configureProjection({jobRuntime: runtime, managedNodes: runtime.managedNodes, harnessEfficiency: runtime.harnessEfficiency, workParcels: runtime.workParcels, modelRegistry: registry, parameterizedJobs, adaptiveOrchestration: runtime.adaptiveOrchestration, capabilityIntelligence: capabilities, modelIntelligence, qualificationSuite, providerCatalog, resources: resources.map(resource => ({id: resource.id, name: resource.name ?? resource.id, platform: resource.platform, transport: resource.transport.type, capabilities: resource.capabilities}))});
