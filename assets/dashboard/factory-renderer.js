...
export async function createFactoryRenderer(host,{onSelect,fallback=false,lowDetail=false,reducedMotion=false}={}){
  ...
  let entities=[],relations=[],layout=positionFactory([]),objects=new Map(),hits=[],selected=null,mode='OVERVIEW',focus=null,caption='',badge='CONNECTING',at='',disposed=false,dirty=true,raf=0,lastPaint=0,transition=0;
  ...
  function model(e){const group=new T.Group(),colour=entityColour(e),station=['worker','model'].includes(e.kind);
    ...
    group.userData.entityId=e.id;return group;
  }
  ...
  function paint(now){if(disposed)return;raf=requestAnimationFrame(paint);const moving=transition>now;if(!dirty&&!moving)return;if(now-lastPaint<(lowDetail?100:33))return;lastPaint=now;const started=performance.now();dirty=false;
    if(mode==='FOLLOW JOB'||mode==='FOLLOW WORKER'){const p=layout.positions.get(focus);if(p)target={x:p.x,y:0,z:p.z};distance=58;}
    ...
    ctx.textAlign='left';ctx.fillStyle='#f3f6fb';ctx.font='600 16px system-ui';ctx.fillText('AGENT CONTROL  /  FACTORY',24,30);ctx.fillStyle='#80a2b9';ctx.font='11px ui-monospace, monospace';ctx.fillText(`${badge}   ${at}   ${fallback?'2D':'3D'} / ${mode}`,24,52);
    const jobs=entities.filter(e=>e.kind==='job');ctx.fillText(`${jobs.length} RUNS   ${entities.filter(e=>e.kind==='worker').length} WORKERS   ${entities.filter(e=>e.kind==='model').length} MODELS   ${entities.filter(e=>e.kind==='evidence').length} EVIDENCE`,24,72);
