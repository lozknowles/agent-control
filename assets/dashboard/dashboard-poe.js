(()=>{
  ...
  function paintState(){const name=poeView.localState||poeView.conversation?.state||'IDLE';q('#poe-character').dataset.state=name;q('#poe-character').dataset.focus=tourIndex>=0?'tour-left':poeView.reference?.kind||poeView.conversation?.lastReference?.kind||'conversation';q('#poe-state').textContent=name.replaceAll('_',' ');q('#poe-interrupt').disabled=!poeView.playback&&!poeView.busy;q('#poe-speak').disabled=!poeView.projection?.voice?.recognition;q('#poe-form button[type="submit"]').disabled=poeView.busy;paintTour();paintPet();}
  async function load(){
    ...
    poeView.loading=(async()=>{
      ...
      sessionStorage.setItem(key,poeView.conversation.id);render();
  ...
  function fail(error){poeView.busy=false;if(tourIndex>=0&&tourSpeech!=='complete')tourSpeech='paused';setLocal('FAILED',`POE could not complete that step: ${error.message}. Typed conversation remains available.`);showError(error)}
  ...
  function positionCompanion(){const pet=q('#poe-launcher');if(!pet)return;const w=pet.offsetWidth||96,h=pet.offsetHeight||116,header=document.querySelector('body>header')?.getBoundingClientRect(),usage=q('#persistent-usage')?.getBoundingClientRect();const start=Math.max(70,header?.bottom||0)+10;
    const obstacles=[...document.querySelectorAll('body>header,#poe-regression-progress,button[data-run-command="approve"],button[data-parcel-command="approve"]')].filter(el=>!el.hidden&&el.getClientRects().length).map(el=>el.getBoundingClientRect());
    if(!petPosition){const candidates=[];for(let y=start;y<innerHeight-h-8;y+=h+8)candidates.push({x:innerWidth-w-18,y});candidates.push({x:18,y:start});petPosition=candidates.find(c=>!obstacles.some(r=>c.x<r.right&&c.x+w>r.left&&c.y<r.bottom&&c.y+h>r.top))||candidates[0]||{x:innerWidth-w-18,y:80};}
  ...
  function paintPet(){const pet=q('#poe-pet-art .poe-character'),name=poeView.localState||poeView.conversation?.state||'IDLE';if(pet){pet.dataset.state=name;pet.dataset.focus=tourIndex>=0?'tour-left':'conversation';}q('#poe-pet-state').textContent=name.replaceAll('_',' ');const latest=[...(poeView.conversation?.turns||[])].reverse().find(t=>t.actor==='poe'),bubble=q('#poe-pet-bubble');bubble.textContent=latest?.text||'';bubble.title=latest?.text||'';bubble.hidden=!q('#poe-workspace').hidden||!latest||(!poeView.playback&&latest.purpose!=='GREETING');positionCompanion();}
  function collapseCompanion(){finishTour();interrupt().catch(fail);endVoice();q('#poe-workspace').hidden=true;q('#poe-launcher').setAttribute('aria-expanded','false');paintPet();}
  ...
  function paintTour(){
    ...
    if(active)q('#poe-tour-position').textContent=`${tourIndex+1}/${tourSteps.length} · ${tourSteps[tourIndex][0]} · ${tourSpeech==='complete'?'Ready for the next feature':tourSpeech==='paused'?'Tour paused — retry narration':tourSpeech==='playing'?'POE is explaining this feature':'Preparing spoken explanation'}`;
  }
  async function tourStep(){
    ...
    await sendText(`[Operator-selected guided tour: ${title}] As Agent Control's part-time tour guide, briefly explain the highlighted ${title} area and what the operator can see or do there. ${question} ${tourIndex===0?'Begin with a brief self-introduction as POE, the part-time Agent Control tour guide. ':''}Use authoritative evidence only, distinguish unavailable features, and keep the spoken explanation to two concise sentences.`,epoch);
  }
  ...
  async function refreshRegression(){if(state.operatorAuth!=='authenticated')return;try{const progress=await request('/api/poe/regression'),panel=q('#poe-regression-progress');if(progress.state==='UNAVAILABLE'){panel.hidden=true;return}panel.hidden=false;const value=n=>n===null||n===undefined?'Unavailable':safe(n);panel.innerHTML=`<strong>Full regression · ${safe(progress.state)}${progress.stale?' · STALE':''}</strong><span>Phase: ${safe(progress.phase)}</span><span>Passed ${value(progress.passed)}</span><span>Failed ${value(progress.failed)}</span><span>Skipped ${value(progress.skipped)}</span><span>Remaining ${value(progress.remaining)}</span><span>Elapsed ${progress.elapsedMs===null?'unavailable':Math.floor(progress.elapsedMs/1000)+'s'}</span><small>Commit ${safe(progress.commit)} · External test runner; not a Work Parcel</small>`;
    const milestone=`${progress.runId}:${progress.phase}:${progress.state}`;if(narrateTests&&tourIndex<0&&!progress.stale&&milestone!==lastRegression&&!poeView.busy&&!poeView.playback&&!poeView.holding){lastRegression=milestone;await load();await sendText('[Operator-enabled regression narration] Explain the current full regression progress and result concisely. State unavailable remaining totals honestly.');}}catch{const panel=q('#poe-regression-progress');if(!panel.hidden)panel.innerHTML='<strong>Regression evidence unavailable</strong>';}}
  function initializeCompanion(){const clone=q('#poe-character').cloneNode(true);clone.removeAttribute('id');clone.querySelectorAll('[id]').forEach(el=>{const old=el.id;el.id=old+'-pet';clone.querySelectorAll('[fill]').forEach(node=>{if(node.getAttribute('fill')===`url(#${old})`)node.setAttribute('fill',`url(#${old}-pet)`)});});q('#poe-pet-art').append(clone);positionCompanion();let drag=null;
    ...
    document.addEventListener('poe:open',openPanel);q('#poe-expand').addEventListener('click',()=>{const expanded=q('#poe-workspace').classList.toggle('poe-expanded-detail');q('#poe-expand').setAttribute('aria-pressed',String(expanded));q('#poe-expand').textContent=expanded?'Compact':'Expand'});
