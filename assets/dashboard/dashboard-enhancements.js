...
function renderTokenRouting(routing) {
  ...
  root.innerHTML = threads.map(thread => {
    ...
    const parcel = (routing.parcels || []).find(item => item.parcelId === thread.parcelId), routeLabel = item => `${item.providerId}/${item.accountLabel || item.accountProfileId || 'default'}/${item.modelId}`, chain = parcel?.byModel?.map(item => `${routeLabel(item)} ${tokenNumber(item.totalTokens)}`).join(' → ') || 'No parcel totals';
