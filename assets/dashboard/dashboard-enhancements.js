...
function statusClass(status) { return ['FAILED', 'DEGRADED', 'DISCONNECTED', 'OFFLINE'].includes(status) ? 'error' : ['BUSY', 'QUEUED', 'WAITING', 'WAITING_FOR_WORKER', 'WAITING_FOR_DEPENDENCY', 'WAITING_FOR_RESOURCE', 'WAITING_FOR_APPROVAL', 'RETRY_PENDING'].includes(status) ? 'waiting' : ''; }
...
function renderRunEfficiency(run) {
  ...
  return `<div class="metrics active-run-telemetry"><div class="metric"><span>Stage</span><strong>${esc(stage)}</strong></div><div class="metric"><span>Provider / model</span><strong>${esc(latest ? `${latest.provider} / ${latest.model}` : 'Unavailable until provider reports')}</strong></div><div class="metric"><span>Elapsed</span><strong data-live-start="${esc(run.startedAt || run.requestedAt)}">${esc(durationLabel(run.startedAt || run.requestedAt, run.endedAt))}</strong></div><div class="metric"><span>Last meaningful activity</span><strong data-live-activity="${esc(run.updatedAt || run.requestedAt)}">${esc(ageLabel(run.updatedAt || run.requestedAt))} ago</strong></div><div class="metric"><span>Input fresh / cached</span><strong>${esc(fresh ?? 'Unavailable')} / ${esc(cached ?? 'Unavailable')}</strong></div><div class="metric"><span>Output / reasoning / total</span><strong>${esc(output ?? 'Unavailable')} / ${esc(reasoning ?? 'Unavailable')} / ${esc(total ?? 'Unavailable')}</strong></div><div class="metric"><span>Accumulated cost</span><strong>${esc(cost === null ? 'Unavailable' : `${latest?.currency || ''} ${cost}`.trim())}</strong></div><div class="metric"><span>Verification</span><strong>${esc(latest?.verifierResult || verification)}</strong></div></div>`;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setInterval(() => { document.querySelectorAll('[data-live-start]').forEach(node => { node.textContent = durationLabel(node.dataset.liveStart); }); document.querySelectorAll('[data-live-activity]').forEach(node => { node.textContent = `${ageLabel(node.dataset.liveActivity)} ago`; }); }, 1000);
});
