...
function renderTokenRouting(routing) {
  ...
  root.innerHTML = threads.map(thread => {
    ...
    const parcel = (routing.parcels || []).find(item => item.parcelId === thread.parcelId), routeLabel = item => `${item.providerId}/${item.accountLabel || item.accountProfileId || 'default'}/${item.modelId}@${item.nodeId || 'controller'}`, chain = parcel?.byModel?.map(item => `${routeLabel(item)} ${tokenNumber(item.totalTokens)}`).join(' → ') || 'No parcel totals';
    const nextDecision = [...(routing.decisions || [])].reverse().find(item => item.threadId === thread.threadId && item.target), nextRoute = nextDecision?.target ? routeLabel(nextDecision.target) : 'none selected';
    ...
    return `<article class="token-thread ${esc(String(thread.governor?.state || 'CONTINUE').toLowerCase())}"><div><strong>${esc(thread.providerId)} / ${esc(thread.accountLabel || thread.accountProfileId || 'default account')} / ${esc(thread.modelId)} @ ${esc(thread.nodeId || 'controller')}</strong><span class="status-pill">${esc(thread.active ? thread.governor?.state || 'CONTINUE' : 'COMPLETED')}</span></div><p>Account plan: ${esc(thread.accountPlan ? `${thread.accountPlan} (${thread.accountPlanAuthority || 'authority unknown'})` : 'not reliably known')} · ${esc(thread.accountQualification || 'not applicable')} / ${esc(thread.accountAvailability || 'default')} · next route ${esc(nextRoute)}</p><p>Context: ${esc(tokenNumber(context.tokens))} / ${esc(tokenNumber(context.limitTokens))} — ${esc(percent)} <small>${esc(context.authority || 'unavailable')}</small></p><p>Context lifecycle: ${esc(lifecycle ? `${lifecycle.kind} · ${lifecycle.authority} · ${lifecycle.source}` : 'no transition observed')}</p><p>Tokens: in ${esc(tokenNumber(point.cumulative?.inputTokens))} · out ${esc(tokenNumber(point.cumulative?.outputTokens))} · total ${esc(tokenNumber(point.cumulative?.totalTokens))}</p><p>Cost: ${esc(tokenMoney(point.cost))} · elapsed ${esc(Math.round(tokenElapsed(thread) / 1000))}s</p><p>Governor: ${esc(thread.governor?.state || 'CONTINUE')} · current ${esc(currentThreshold)} → next ${esc(nextThreshold)}</p><p class="token-parcel">Parcel: ${esc(chain)} = ${esc(tokenNumber(parcel?.totalTokens))} total</p></article>`;
  }).join('');
...
function renderParcelAudit(parcel) {
  ...
  const invocations = audit.invocations.length ? audit.invocations.map(item => { const accounting = item.costAccounting, price = accounting?.cloud?.pricingBasis, local = accounting?.localEnergy; const costDetail = accounting?.billingMode === 'SUBSCRIPTION_QUOTA' && item.calculatedCost === null && item.providerReportedCost === null ? `SUBSCRIPTION / QUOTA CONSUMPTION · ${accounting.subscription?.unitsConsumed ?? 'unknown'} ${accounting.subscription?.unitLabel ?? 'units'} · reset ${accounting.subscription?.resetAt ?? accounting.subscription?.resetPeriod ?? 'unknown'}` : price ? `${price.currency} input ${price.inputPerMillionTokens}/M · cached ${price.cachedInputPerMillionTokens ?? 'unpriced'}/M · output ${price.outputPerMillionTokens}/M · ${price.tableId}@${price.version} (${price.source})` : item.costBasis; const localDetail = local ? ` · local ${local.energyWh ?? 'unknown'} Wh / ${local.estimatedElectricityCost ?? 'unknown'} ${local.currency ?? ''} (${local.estimate ? 'estimate' : 'measured'})` : ''; const identity = item.providerModel ? `${item.providerModel} · role ${item.logicalRole || 'explicit'} · qualification ${item.qualificationVersion}` : item.profile; const cache = item.cacheEvidence ? `cache reused ${item.cacheEvidence.reusedTokens ?? 'unavailable'} · processed ${item.cacheEvidence.processedPromptTokens ?? 'unavailable'} · ${item.cacheEvidence.authority}` : 'cache evidence unavailable'; return `<tr><td>${esc(item.provider)}<br><small>${esc(item.route)}</small></td><td>${esc(item.registryModelId || item.model)}<br><small>${esc(identity)}</small><br><small>${esc(cache)}</small></td><td>${esc(item.node ?? 'Not reported')}</td><td>${esc(item.freshInputTokens ?? 'Not reported')} / ${esc(item.cachedInputTokens ?? 'Not reported')}</td><td>${esc(item.outputTokens ?? 'Not reported')} / ${esc(item.reasoningTokens ?? 'Not reported')}</td><td>${esc(item.totalTokens ?? 'Not reported')}</td><td>${esc(item.providerReportedCost ?? item.calculatedCost ?? 'Not reported')}<br><small>${esc(costDetail + localDetail)}</small></td><td>${esc(durationLabel(item.startedAt, item.completedAt))}</td></tr>`; }).join('') : '<tr><td colspan="8">No model invocation recorded. Deterministic control actions consume no model tokens.</td></tr>';
...
function renderParcels() {
  ...
  document.querySelector('#parcel-list').innerHTML = jobState.parcels.length ? jobState.parcels.map(parcel => {
    ...
    return `<article class="parcel-card ${parcel.status === 'RUNNING' ? 'is-running' : ''}"><div class="parcel-head"><strong>${esc(parcel.objective)}</strong><span class="status-pill ${statusClass(parcel.status)}">${esc(parcel.status)}</span><p>${esc(parcel.prompt)}</p></div><div class="parcel-metrics"><span>${esc(parcel.planner.kind)} planner</span><span>${esc(durationLabel(parcel.createdAt, parcel.endedAt))}</span><span>tokens ${esc(t.totalTokens ?? 'unavailable')}</span><span>cost ${esc(t.cost === null ? 'unavailable' : `${t.currency || ''} ${t.cost}`.trim())}</span></div>${integrity}${summary}${live}${decision}${stages}${renderParcelAudit(parcel)}</article>`;
...
function renderRunEfficiency(run) {
  ...
  const pending = latest?.state === 'RUNNING', fresh = pending ? 'awaiting completion' : known(item => item.usage.freshInputTokens) ? sum(item => item.usage.freshInputTokens) : null, cached = pending ? 'awaiting completion' : known(item => item.usage.cachedInputTokens) ? sum(item => item.usage.cachedInputTokens) : null, output = pending ? 'awaiting completion' : known(item => item.usage.outputTokens) ? sum(item => item.usage.outputTokens) : null;
