...
function renderRunEfficiency(run) {
  ...
  const cost = pending ? 'awaiting completion' : values.length > 0 && values.every(item => item.providerReportedCost !== null) ? sum(item => item.providerReportedCost) : values.length > 0 && values.every(item => item.calculatedCost !== null) ? sum(item => item.calculatedCost) : null;
