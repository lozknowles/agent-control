...
function runLink(parent,id){if(!id)return text(parent,'—','span');const a=document.createElement('a');a.href=`/?messagingRun=${encodeURIComponent(id)}`;a.textContent=id;parent.append(a);}
...
async function refresh(){
  ...
  table(el('commands'),['When','Sender','Command','Outcome','Job'],current.commands||[],(tr,c)=>{text(tr,new Date(c.at).toLocaleString(),'td');text(tr,c.sender,'td');text(tr,c.verb,'td');text(tr,`${label(c.state)}${c.code?` · ${label(c.code)}`:''}`,'td');runLink(text(tr,'','td'),c.runId);});
  table(el('deliveries'),['Message','Job','State','Attempts','Recovery'],current.deliveries||[],(tr,d)=>{text(tr,`#${d.id} ${d.kind}`,'td');runLink(text(tr,'','td'),d.runId);text(tr,`${label(d.state)}${d.state==='submitted'?' (delivery unconfirmed)':''}`,'td');text(tr,String(d.attempts),'td');const cell=text(tr,'','td');if(['uncertain','failed'].includes(d.state))button(cell,'Retry message',()=>{if(d.state==='uncertain'&&!confirm('This message may already have arrived. Retry and accept possible duplicate delivery?'))return;return api('/retry',{id:d.id,acknowledgeUncertain:d.state==='uncertain'});});else text(cell,label(d.code),'span');});
}
