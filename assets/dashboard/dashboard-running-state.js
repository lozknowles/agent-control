(function attachAgentControlRunningState(root) {
  const terminal = new Set(['SUCCEEDED', 'FAILED', 'DEGRADED', 'BLOCKED', 'CANCELLED', 'PAUSED', 'MISSED', 'DISCONNECTED']);
  ...
  const liveness = (status, updatedAt, currentTime = Date.now(), staleAfterMs = 120000) => {
    if (status !== 'RUNNING') return {active: false, stale: false, ageMs: null, label: status === 'WAITING' ? 'Waiting; not actively executing' : 'Not actively executing'};
  ...
  const usage = telemetry => {
    const fields = [telemetry?.freshInputTokens, telemetry?.cachedInputTokens, telemetry?.outputTokens, telemetry?.reasoningTokens, telemetry?.totalTokens, telemetry?.cost];
    if (fields.every(value => value === null || value === undefined)) return {available: false, label: 'Usage reported on completion'};
    return {available: true, label: `input ${metric(telemetry?.freshInputTokens)} · cached ${metric(telemetry?.cachedInputTokens)} · output ${metric(telemetry?.outputTokens)} · reasoning ${metric(telemetry?.reasoningTokens)} · total ${metric(telemetry?.totalTokens)} · cost ${metric(telemetry?.cost)}`};
  };
  const rollup = stages => {
