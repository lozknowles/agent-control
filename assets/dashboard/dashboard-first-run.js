(() => {
  ...
  async function activate(){
    ...
    try{
      ...
      byId('home-next').textContent=discovery.progress?.state==='RUNNING'?'Discovery is running. Follow each check in Environment Discovery.':scan?`Discovery ${scan.status.toLowerCase()}. ${scan.summary.needsQualification??0} resources need qualification. Open Estate to inspect current availability.`:'No discovery has run yet. Start with this computer; remote machines are optional.';
