...
- Android local wireless-ADB pairing/reconnect is implemented and deterministically tested, but the 3.9 physical gate is incomplete: the qualification Pixel had neither a discovered pairing/connect endpoint nor an available locally entered PIN. The node therefore correctly withheld `android.adb.local` and `transport.adb`.
- Linux process-group cleanup is physically qualified. The Windows process-tree adapter has deterministic coverage but was not physically cancelled during this candidate qualification; unsupported substrates report cleanup uncertainty.
- The real Codex cache-boundary comparison is observational. The available Codex CLI does not expose explicit prompt-cache controls, cache-write counts, authoritative current-context occupancy or billed cost, so no repeatable saving is claimed. Explicit cache controls remain capability-gated on Responses adapters.
