...
The optional presentation in **Crew** adds six original, navigable operational characters beside Lanes, task entry, Work Parcels, Models, Systems and Run evidence. Their poses, text, counts and warnings are a deterministic projection of canonical state—not autonomous agents or generated status prose. Mixed conditions remain visible as badges, stale telemetry never becomes a fabricated failure, and completion animation is suppressed on load/reconnect. In Full motion, freshly idle characters look around and sustained-idle characters sleep with a gentle breathing/wake cycle; the operational state remains `idle`. Full, reduced, off and hidden settings are browser-local; system reduced-motion is respected. See [dashboard operational characters](docs/dashboard-characters.md) for the roster, exact mappings, inspection gallery, accessibility controls and rollback.
