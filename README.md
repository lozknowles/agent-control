...
> 4.4 development note: **Your Memories** is the user-facing Agent Control capability for finding relevant prior context, checking its governance and provenance, rejecting stale or unrelated memories, and safely supplying accepted context to the current session. In an isolated physical experiment, the same local model retained 8/8 correct answers while supplied context fell 90.87% and provider input fell 79.50%; retrieval latency increased and monetary saving remained unavailable. Memories remain advisory and cannot override Work Parcels, batons, policy, authoritative evidence or execution state. Technical architecture and optional-backend details remain in the [memory architecture review](docs/memory-architecture-review.md) and [physical evidence](docs/evidence/agent-control-4.4-marm-memory-qualification-20260911.md). No 4.4 memory runtime is released from this branch.
