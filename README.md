...
The opt-in real-mutation benchmark goes further: a live model receives only typed, policy-gated repository mutation tools and modifies a fresh disposable Git fixture. Hidden deterministic verifiers, public regressions, scope rules, syntax, `git diff --check`, and credential/topology scans decide success independently of model claims. The current sealed suite contains 24 satisfiable tasks, including nine held-out tasks, and compares STANDARD with explainable predicted THIN/STANDARD/DEEP selection plus bounded escalation using the same execution model. The final run verified 24/24 for both strategies and 9/9 held-out for both. Automatic routing nevertheless remains disabled: one recovered timeout made cumulative candidate token use unknown, and median latency improved only 1.2%, below the predeclared 5% production gate. See [`docs/real-mutation-routing-qualification-20260828.md`](docs/real-mutation-routing-qualification-20260828.md) and [`artifacts/real-mutation-routing-qualification-20260828.json`](artifacts/real-mutation-routing-qualification-20260828.json).
