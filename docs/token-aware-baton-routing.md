...
In 3.8 development, the same provider-neutral baton may include content-addressed Evidence Packet references. A receiving model can rehydrate only evidence that still matches repository identity and freshness, instead of receiving repeated raw context. An adapter-private index, provider session or native context handle is never the only continuation representation.
