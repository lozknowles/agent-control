...
Synthetic tests cover legacy/new pre-disruption proof, unknown evidence, requested/acknowledged failures, immutable parent files, linked identity, invalid/expired/wrong-target authority, old authority non-reactivation, fresh prechecks, wrong identity, unhealthy service, concurrency across instances, idempotency, retained quarantine and preparation-only zero port calls. A synthetic complete recovery and failing dispatch verify the write-ahead boundary. Sentinel benchmark results/ownership remain untouched; real qualification independently hashes the actual retained run artifacts and locks.
