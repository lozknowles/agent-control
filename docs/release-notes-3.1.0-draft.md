...
Generic Android devices can now be discovered by observed secure-overlay platform without a hostname, model, serial or address in policy. Tailscale DERP and direct paths are both reachable network states, while endpoint health and authenticated typed-job capability remain separate gates. The existing Termux observer uses the shared typed-node contract, and a native foreground adapter adds an explicitly approved read-only `nfc.inspect_tag` capability with visible `WAITING_FOR_CARD`, bounded timeout/cancellation and normalized raw identifier evidence. It exposes no shell, APDU, NFC transceive, authentication, write, cloning or emulation operation. Source/build support is not a live device or card-read claim.
