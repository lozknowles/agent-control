...
Commands: `help`, `jobs`, `run <template> {"argument":"value"}`, `status <run-id>`, `cancel <run-id>`, `report <run-id>`, `watch <run-id>`, `unwatch <run-id>`. Use the stable runtime run ID, not the template ID. Jobs and cancellations are scoped to jobs started by that enrolled sender. Pause/resume are unsupported. No approval command exists; use the authenticated main dashboard.
