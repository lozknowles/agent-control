...
## Integration with active 4.5 work

This change is prepared in `feature/morrow-host-identity`, based on main commit `2e74d88db57e3ad15ce1d85ec93220d087b9592f`. It is independent of the active release-gate completion branch. No memory, specialist-learning, routing, energy qualification, package version or release-gate result is changed.

Review and integrate through the normal repository process when compatible with the active work. A deployment requires the normal controlled service update; the development branch does not change the currently running host. Rollback is a normal revert of the identity commit, with no database migration.
