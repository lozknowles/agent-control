...
The **Models** view reads `GET /api/models/accounts` and model account projections. It shows account friendly label, plan plus metadata authority, availability and independent qualification state. **Check account** invokes the selected profile's bounded `codex login status` using only its isolated child-process `CODEX_HOME`. The dashboard never receives credential environment names, resolved paths, email addresses, OAuth tokens, cookies, or authentication-file contents.
