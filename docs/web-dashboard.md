...
The default **Jobs** area contains four separate platform views: **Job Definitions**, **Saved Jobs**, **Schedules**, and **Runs**. These sit alongside the existing catalog/Run-ledger projection rather than replacing its Action/DAG workflows. **Lanes** retains the interactive multi-agent control room. **Sessions** projects persistent Actor, participant, delegation, ACP, contract/PTY, handoff, model/runtime and evidence identity. **Systems** shows canonical configured inventory plus ACP transport and lifecycle-recipe readiness. **Models** shows the canonical provider-neutral model registry and immutable 3.6 lifecycle state. **Routing** shows the evidence-conditioned Model Capability League, Workflow League and persisted per-Work-Parcel Decision Tree. **Warm Experts** shows temporary cache-aware route expertise and routing decisions. **Configuration** provides authenticated, validated inventory and fast-execution/adaptive/cache policy editing. No view parses terminal text for Run state.

## Warm Experts

Open **Warm Experts** to inspect the runtime Cache Expert Registry and its durable routing decisions. Each card identifies the worker/crew route, provider/account/model/node, session/cache/backend scope, task domain, repository reference, lifecycle and age, actual reused and processed tokens, expected next reuse, evidence authority, health, recent verified history and sourced economics. Records with no reliable cache evidence show `CACHE STATE UNKNOWN`; missing pricing shows `MONETARY SAVING UNAVAILABLE`.

Decision cards expose every candidate’s governed eligibility, `EXACT`/`HIGH`/`PARTIAL`/`INCOMPATIBLE`/`UNKNOWN` compatibility, base/cache/total score, estimated context delta, selected route, route change and independent decision-verifier result. Expanding **Human-readable routing transcript** displays the full natural operational record from the same durable decision—not a generated summary and not private model reasoning. Work Parcel SSE events trigger a fresh projection during execution; periodic reconciliation protects against a missed event.
...
The **Models** view reads `GET /api/models/accounts` and model account projections. It shows account friendly label, execution node, plan plus metadata authority, availability and independent qualification state. **Check account** dispatches bounded `codex login status` to the selected profile's configured node; `CODEX_HOME` is resolved only there. The dashboard never receives credential environment names, resolved paths, full executable paths, raw process output, email addresses, OAuth tokens, cookies, or authentication-file contents.
