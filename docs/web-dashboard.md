...
The **Retrieval** panel reads the redacted `retrieval` status projection and updates through the existing SSE stream on `retrieval.started`, `retrieval.escalated`, `retrieval.evidence`, and `retrieval.failed`. It shows provider/strategy path, calls and escalation, evidence items/tokens, raw bytes avoided, estimated context savings, freshness, locality and latency. Evidence text, repository roots, index paths and raw provider output are excluded. `GET /api/retrieval` is read-only. See [governed retrieval](governed-retrieval.md).
