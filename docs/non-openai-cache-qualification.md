...
The provider reported neither cache-write counts nor TTFT. No tariff or measured energy input existed for this local route, so monetary savings remain unavailable. Cache population is included because every warm arm follows and is paired with its cold population arm. Warm-expert delegation remains not implemented: provider slot reuse does not prove that production routing selects an agent because it has relevant retained context.
