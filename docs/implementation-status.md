...
| Governed local Android wireless-ADB reliability (`android.local-adb-reliability`) | **PARTIAL** | A bounded helper serializes pairing/reconnect ownership, recovers stale attempts, accepts pairing PIN only through local stdin, discovers pairing/connect DNS-SD services, preserves stable device identity, distinguishes paired from usable, verifies connection independently and publishes ADB transport capability only while qualified. | The candidate Pixel had no discovered local pairing/connect endpoint and no available locally entered PIN. Pair, reconnect and execution/resume were therefore not physically exercised, and android.adb.local/transport.adb correctly remained unpublished. |
| Portable prompt-cache boundary and truthful accounting (`providers.portable-prompt-cache-boundary`) | **PARTIAL** | Provider-neutral stable/volatile prompt blocks preserve deterministic rendering. Responses adapters add hashed cache keys or explicit breakpoints only when provider and model both advertise the qualified capability; cache reads, writes, fresh/total input and configured costs remain separate through execution and dashboard accounting. | The physical Codex CLI route exposes automatic cache-read observations but not explicit Responses controls, cache-write counts, authoritative current context or billed cost. The alternating comparison is observational and does not establish repeatable token, latency or monetary savings. |
