...
| --- | --- | --- | --- |
| Optional governed OpenWA messaging pilot (`messaging.openwa-pilot`) | **PARTIAL** | Signed direct commands, authenticated two-step enrolment, immutable template grants, durable runtime request keys, outbound recovery and private dashboard setup are implemented and automatically tested. | Dedicated account linking and separate operator enrolment are pending. No physical round trip, conversation video or real model handoff is qualified. Legacy jobs and pinned saved reviews are supported; the pilot has no qualified review provider and the benchmark capability remains gated. |
...
- Qualification evidence: [`docs/openwa/README.md`](../docs/openwa/README.md)
