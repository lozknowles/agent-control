...
Release boundary: **4.3.0-development**. Registry updated: **2026-09-09**.
...
| Token-aware baton routing and live telemetry (`routing.token-aware-baton`) | **QUALIFIED** | Provider-neutral thread telemetry keeps cumulative lifetime tokens separate from current context occupancy, persists configurable 60/75/85/90 governor transitions and context-lifecycle boundaries, sealed baton provenance, failed-handoff recovery and additive Work Parcel provider/account/model/node-chain totals. Optional governed account profiles bind exact model routes to isolated Codex homes on a configured execution node without persisting credentials or resolved paths. A restricted Codex node port reuses configured Resources and fixed-script SSH for Windows account qualification and read-only structured execution. The normal parameterized repository-review lifecycle assesses immutable chunk boundaries, delegates an account-qualified destination through existing contract/handoff runtimes and returns results to independent validation. The redacted dashboard/API projection streams telemetry and context transitions over existing SSE without a page reload. | A bounded production Work Parcel physically qualified live SSE telemetry, a sealed source-to-destination baton, destination continuation, independent verification, additive provider/model accounting and original-thread recovery after an injected destination refusal. Both routes were distinct local OpenAI-compatible provider/model configurations; authoritative current-context occupancy and source monetary cost remain unavailable where the providers do not expose them. Remote Codex account qualification passed separately, but Codex was not a workload leg in this lifecycle proof. |
...
| Cache-Aware Expert Delegation (`routing.cache-aware-experts`) | **PARTIAL** | A provider-neutral runtime registry derives temporary Warm Experts from genuine terminal invocation telemetry, binds evidence to exact worker/provider/account/model/node/session/cache/backend identity and hashed context, enforces strict compatibility and lifecycle decay, and contributes a bounded cache score only after capability, placement, health and transport-integrity gates. Normal Work Parcels persist candidate decisions, selected expert identity, batons, actual cache observations and independent verification; the dashboard/API exposes the same durable facts and natural operational transcript. | Deterministic implementation is present, but the required physical A-F non-OpenAI experiment, independent evidence reconciliation and genuine HD recording have not yet passed. Cache reuse alone is not accepted as proof that delegation changed. |
...
- Qualification evidence: [`docs/cache-aware-expert-delegation.md`](../docs/cache-aware-expert-delegation.md), [`docs/non-openai-cache-qualification.md`](../docs/non-openai-cache-qualification.md)
