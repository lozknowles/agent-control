...
| Harness efficiency telemetry and context profiles (`harness.efficiency-context-routing`) | **PARTIAL** | Provider-neutral invocation telemetry, THIN/STANDARD/DEEP profiles, context packets, a neutral context-graph port, typed real-mutation execution, deterministic diff verification, held-out outcomes and dashboard/API projections are executable; routing stays observational with STANDARD applied. | The sealed 24-task run verified 24/24 STANDARD and 24/24 predicted outcomes, including 9/9 held-out for both, but candidate median latency improved only 1.2% and a recovered timeout left cumulative candidate token use unknown. The 5% resource-or-latency production gate did not pass. |
...
- Qualification evidence: [`docs/real-mutation-routing-qualification-20260828.md`](../docs/real-mutation-routing-qualification-20260828.md), [`artifacts/real-mutation-routing-qualification-20260828.json`](../artifacts/real-mutation-routing-qualification-20260828.json)
