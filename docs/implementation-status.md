...
Release boundary: **3.7.0-unreleased**. Registry updated: **2026-09-03**.
...
| Token-aware baton routing and live telemetry (`routing.token-aware-baton`) | **IMPLEMENTED** | Provider-neutral thread telemetry keeps cumulative lifetime tokens separate from current context occupancy, persists configurable 60/75/85/90 governor transitions and context-lifecycle boundaries, sealed baton provenance, failed-handoff recovery and additive Work Parcel provider/account/model/node-chain totals. Optional governed account profiles bind exact model routes to isolated Codex homes on a configured execution node without persisting credentials or resolved paths. A restricted Codex node port reuses configured Resources and fixed-script SSH for Windows account qualification and read-only structured execution. The normal parameterized repository-review lifecycle assesses immutable chunk boundaries, delegates an account-qualified destination through existing contract/handoff runtimes and returns results to independent validation. The redacted dashboard/API projection streams telemetry and context transitions over existing SSE without a page reload. | Both isolated remote Codex accounts physically passed the production account-status path after correcting stdin framing and native stderr classification. The first bounded structured workload then timed out before returning provider telemetry, so the live baton, destination continuation, physical mismatch, recovery and ledger-reconciliation gates remain unproven. Authoritative context and cost remain unavailable where providers do not report them; the physical verdict remains PARTIAL. |
