# Upgrade Agent Control 4.6.1 to 4.7
...
3. Clone `v4.7.0` into a clean sibling directory; never pull over a dirty installation. Run the documented bootstrap with the existing supported Node.js runtime.
...
For rollback, stop 4.7, preserve its logs and evidence, and restart the saved 4.6.1 checkout against the pre-upgrade backup. Do not erase the new history or overwrite the old backup. Release publication does not deploy to your estate automatically.

The release qualification performs clean Linux installation and an isolated upgrade from the exact formal 4.6.1 source. It does not imply a new Windows, macOS or Android installation qualification.
