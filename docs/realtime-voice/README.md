...
RealtimeVoiceSession receives ordered 16kHz mono PCM16 frames of 20ms. A bounded experimental energy VAD detects voice and normally ends an utterance after 500ms silence. A 10-second maximum endpoint prevents constant background energy from leaving a turn open forever, then waits for real silence before accepting another utterance. A gap discards the partial utterance; duplicates and malformed frames are rejected. Calls are capped at 10 minutes, conversational history at 100 messages, TTS output at 60 seconds and tool calls at four per turn. These are resource/heuristic settings, not acoustically qualified PASS thresholds.
...
The exact commands `repeat`, `repeat that`, `say it again`, `say that again` and `again` are deterministic conversation controls. They replay the last completed assistant turn through the configured voice without invoking the reasoning model or any tool, and record separate REPEAT REQUEST and REPEAT RESPONSE events. With no earlier assistant turn, the control returns a fixed explanation. Common acknowledgements, the connection check and the bounded one-to-ten test are also deterministic, audited controls. They cannot create, steer, cancel or duplicate work, and their explicit allowlisted responses may use the content-addressed speech cache.
...
Each authenticated transcript download is an Agent Control work-audit record. It begins with the full chronological human-readable conversation, including every final STT utterance and complete spoken model response plus correlated tool, handoff, interruption and call-boundary entries. The detailed immutable event audit and session summary follow it. Hidden reasoning is neither requested nor stored. Transcripts also retain USER SPEECH detection, TTS FIRST AUDIO, TURN COMPLETED/FAILED and other diagnostic events. p50/p95 are withheld below 20 observations, which is a display policy rather than a release threshold. Transport acknowledgement time must not be described as measured handset acoustics. Raw model/tool output must already satisfy the existing redaction rules before it enters the orchestration port.
