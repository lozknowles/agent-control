...
4. Set RAM/storage policy for your model. `minimumAvailableBytes` is the initial available-memory floor; `launchMinimumAvailableBytes` is a greater/equal floor checked again after explicitly authorised service reclamation. They are not inferred model-size estimates. Review context, batch, threads, token ceiling and timeouts. Unknown required telemetry refuses admission.
