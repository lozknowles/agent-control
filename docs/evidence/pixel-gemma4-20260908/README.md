...
The final transcript-scroll observation reached 10,196 px of 10,928 px with a 732 px viewport, remained at the bottom across refresh, and recorded no browser errors.

Final in-flight disconnection: `e2b-disconnect-inflight-01` began a real model request before closing the owned private forward. Pixel runtime logged task launch, cancellation and slot release. The proxy recorded the interrupted request and one refused retry; the job finished FAILED with no fallback. `e2b-cancel-02` repeats dashboard cancellation with bounded normal-runtime reconciliation/capture; it preserves DISCONNECTED with cancellation cleanup unproven and does not invent terminal cancellation. The device-side abort log is separate from authoritative Agent Control cancellation acknowledgment.

The cancellation-02 recording failed its scroll-position assertion for the unresolved live transcript. The durable full transcript is retained and hash-checked; that video is not accepted as complete transcript-reading proof. The successful terminal job recording 05 passed the scroll check.
