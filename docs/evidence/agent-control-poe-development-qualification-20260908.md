...
4. The first normal dashboard demonstration completed both real model legs and independent verification, but its fail-closed recorder found that an authenticated dashboard submission did not carry the safe governed request-origin object into the nested repository-review transcript. Dashboard Work Parcels already retained actor/session attribution, but the complete model transcript could not show the top-level initiating request. The dashboard ingress now creates opaque SHA-256 message/identity references, records `dashboard` modality, `operator-token` authentication class and `parcel.create` authority, and carries that origin through the Work Parcel into the parameterised Run. The bearer token is never retained. Focused web/transcript regression passed 44/44 before the complete 1,039-test gate passed.
