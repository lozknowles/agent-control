...
const envelopeSchema = z.object({
  schema:z.literal(ESTATE_REMOTE_SCHEMA), method:z.literal(ESTATE_REMOTE_METHOD),
...
export function estateProbeProgram(resourceAlias:string,nonce:string) {
...
export function validateEstateEnvelope(text:string,alias:string,nonce:string,expected?:string,controller?:string|null):RemoteEstateEnvelope {
...
export function estateBindingReason(resource:ResourceConfig|undefined):string|null {
  ...
  if(resource.platform!=='linux'||resource.transport.type!=='ssh')return 'UNSUPPORTED_RESOURCE_TRANSPORT';
  if(resource.managedNode?.enabled!==true||binding?.enabled!==true)return 'RESOURCE_NOT_ENABLED_FOR_ESTATE';
...
export function estateSshArgs(resource:ResourceConfig) {
  ...
  return ['-o','StrictHostKeyChecking=yes','-o','UpdateHostKeys=no','-o','ConnectionAttempts=1','-o','ServerAliveInterval=2','-o','ServerAliveCountMax=2',
    ...sshResourceArgs(resource,['timeout','--signal=TERM','--kill-after=2',String(ESTATE_REMOTE_LIMITS.commandSeconds),'python3','-'])];
}
...
export class EstateRemoteAdapter {
  ...
  async discover(resourceId:string,context:EstateRemoteContext):Promise<RemoteEstateResult> {
    ...
    try {
      const response=await Promise.race([cancelled,this.executor('ssh',estateSshArgs(resource!),estateProbeProgram(alias,nonce),{
      ...
      const envelope=validateEstateEnvelope(response.stdout,alias,nonce,resource!.estateDiscovery!.expectedIdentitySha256,context.controllerIdentitySha256);
