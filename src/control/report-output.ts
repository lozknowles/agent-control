...
export function renderReportOutput(input:ReportSource,profileId?:string,format?:string){
 ...
 const jsonOutput=profile.view==='evidence'?{schema:'agent-control.report-evidence/v1',runId:source.id,jobId:source.jobId,sourceSha256:catalogue.sourceSha256,derived:true,source}:{schema:'agent-control.report-output/v1',runId:source.id,jobId:source.jobId,sourceSha256:catalogue.sourceSha256,derived:true,profile:profile.id,view:profile.view,title:source.title,status:source.status,recordedAt:source.recordedAt,contentMarkdown:body};
 let content=selectedFormat==='json'?JSON.stringify(jsonOutput,null,2):head+body+footer;
