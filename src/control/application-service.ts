import {describeInstructionManifests} from './instruction-observer.js';
...
export class AgentControlService {
  ...
  parcelInstructions(id: string) {
    const coordinator=this.mustWorkParcels(), parcel=coordinator.get(id);
    const stores=[...new Set([coordinator.store.instructions,coordinator.runtime.instructions])];
    const manifests=[...new Map(stores.flatMap(store=>store.list(id)).map(manifest=>[manifest.id,manifest])).values()];
    const observations=stores.flatMap(store=>store.events().filter(event=>event.parcelId===id || event.parcelId==='unattributed'));
    const unavailableManifestIds=(parcel.instructionManifestIds??[]).filter(manifestId=>!manifests.some(item=>item.id===manifestId));
    return {schema:'agent-control.instruction-inspection/v1',parcelId:id,mode:'SHADOW',manifests,unavailableManifestIds,observations,
      digest:`${describeInstructionManifests(manifests)} ${observations.filter(event=>event.kind==='SHADOW_ERROR').length} shadow observation error(s); ${unavailableManifestIds.length} referenced manifest(s) unavailable. Evidence: /api/parcels/${encodeURIComponent(id)}/instructions.`};
  }
