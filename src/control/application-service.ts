...
export class AgentControlService {
  ...
  runInspector(id:string,operationId?:string) {
    ...
    if(job){const map=this.runtimeRunMap(id),estate=this.estateMap(),sessions=this.executionSessions?.list()??[],nodes=nodeWorkIndex([],[],sessions,[job]).map(w=>w.nodeId);
      ...
      const inspector=projectJobInspector(job,map,usage,estate,nodes,operationId);return {...inspector,parentUsage:null,runScope:null,sessions:this.executionSessionProjection().filter(s=>s.scope.runId===id),parentRunId:null,history:inspectorHistory(inspector),historyScope:'Derived human-readable export of durable Job Run records',siblingParcels:[]};
    }
