...
export function projectParameterizedRunHistory(input: {run: ParameterizedJobRun; savedJob?: SavedJob; parcels: WorkParcel[]; tokenEvidence?: TokenHistoryEvidence}): ExecutionHistoryProjection {
  ...
  if (run.usage.totalTokens !== undefined || run.usage.cost !== undefined) {
    ...
    const parcelInput = parcelTotal.inputTokens === null
      ...
      : `${number(parcelTotal.inputTokens)} input`;
    add({id: `${run.id}:ledger`, at: run.completedAt ?? run.transitions.at(-1)?.at ?? run.requestedAt, actor: 'SYSTEM EVENT', type: 'LEDGER_RECONCILIATION', title: reconciled ? 'Job and Work Parcel accounting reconcile' : 'Accounting reconciliation incomplete', content: `Job ledger: ${number(run.usage.inputTokens)} input + ${number(run.usage.outputTokens)} output = ${number(run.usage.totalTokens)} total; ${money(run.usage.cost ?? null, run.usage.currency ?? null)}. Work Parcel ledger: ${parcelInput} + ${number(parcelTotal.outputTokens)} output = ${number(parcelTotal.totalTokens)} total; ${money(parcelTotal.cost, parcelTotal.currency)}.`, outcome: reconciled ? 'SUCCEEDED' : 'UNAVAILABLE', jobRunId: run.id, evidenceRefs: run.workParcelIds});
  }
...
function sumParcelUsage(parcels: WorkParcel[]) { const sum = (values: Array<number | null>) => values.length && values.every((value): value is number => value !== null) ? values.reduce((total, value) => total + value, 0) : null; const currencies = [...new Set(parcels.map(parcel => parcel.telemetry.currency).filter((value): value is string => Boolean(value)))]; const freshInputTokens = sum(parcels.map(parcel => parcel.telemetry.freshInputTokens)); const cachedInputTokens = sum(parcels.map(parcel => parcel.telemetry.cachedInputTokens)); return {freshInputTokens, cachedInputTokens, inputTokens: freshInputTokens === null || cachedInputTokens === null ? null : freshInputTokens + cachedInputTokens, outputTokens: sum(parcels.map(parcel => parcel.telemetry.outputTokens)), totalTokens: sum(parcels.map(parcel => parcel.telemetry.totalTokens)), cost: sum(parcels.map(parcel => parcel.telemetry.cost)), currency: currencies.length === 1 ? currencies[0] : null}; }
