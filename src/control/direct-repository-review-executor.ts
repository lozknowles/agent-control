...
export class DirectRepositoryReviewExecutor implements RepositoryReviewExecutor {
  ...
  async execute(request: ReviewExecutionRequest): Promise<ReviewExecutionResponse> {
    ...
    for (const chunk of request.contextChunks) {
      ...
      try {
        ...
        const invocation = await client.invoke(model, prompt, {structured: true, outputSchema: REPOSITORY_REVIEW_OUTPUT_SCHEMA, maximumOutputTokens: request.maximumOutputTokens, timeoutMs: request.run.definition.budgets.timeoutMinutes * 60_000, signal: request.signal, onTelemetry: event => this.tokenRouting?.observe({threadId, parcelId: parcel.id, agentId: request.route.nodeId, providerId: event.providerId, modelId: event.modelId, elapsedMs: event.elapsedMs, active: event.phase === 'started', cumulative: {inputTokens: event.usage?.inputTokens, outputTokens: event.usage?.outputTokens, totalTokens: event.usage?.totalTokens}, context: event.context, cost: {amount: event.usage?.providerReportedCost ?? event.usage?.calculatedCost ?? null, currency: event.usage?.currency ?? null, authority: event.usage?.providerReportedCost === null || event.usage?.providerReportedCost === undefined ? event.usage?.calculatedCost === null || event.usage?.calculatedCost === undefined ? 'unavailable' : 'estimated' : 'authoritative', source: event.usage?.providerReportedCost === null || event.usage?.providerReportedCost === undefined ? event.usage?.calculatedCost === null || event.usage?.calculatedCost === undefined ? 'provider_not_reported' : 'configured_pricing' : 'provider_usage'}})});
  ...
  recordVerification(workParcelIds: string[], verdict: RepositoryReviewResult['verdict']) {
    ...
    for (const id of workParcelIds) {
      const parcel = this.parcels.get(id); if (!parcel || parcel.executionOwner !== 'direct-repository-review-executor') continue;
      ...
      parcel.provenance.push({at, type: 'verification.completed', detail: verdict}); this.parcels.update(parcel);
    }
  }
  ...
  private finishParcel(parcel: WorkParcel, status: 'SUCCEEDED' | 'FAILED', detail: string) { const at = new Date().toISOString(); parcel.status = status; parcel.endedAt = at; parcel.stages[0].status = status; parcel.stages[0].endedAt = at; if (status === 'FAILED') parcel.stages[0].error = detail; parcel.provenance.push({at, type: status === 'SUCCEEDED' ? 'provider-completed' : 'failed', detail}); this.parcels.update(parcel); }
}
