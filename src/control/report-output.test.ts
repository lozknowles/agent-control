...
test('JSON format preserves the selected simple projection rather than silently exporting full evidence',()=>{const s=source();s.profiles=[{id:'quick-json',label:'Quick JSON',view:'simple',format:'json',default:true}];const out=JSON.parse(renderReportOutput(s).content);assert.equal(out.view,'simple');assert.match(out.contentMarkdown,/One correction required/);assert.equal(out.source,undefined);assert.doesNotMatch(JSON.stringify(out),/Exact history|event-1/);});
