...
function fixture(options:{tamper?:boolean;boundedRecovery?:boolean;approvalInside?:boolean}={}){
  ...
  const current={version:'4.6.1',sourceCommit:'a'.repeat(40),sourceDigest:'b'.repeat(64)},checks:Record<string,unknown>={};
  ...
  const receipt={schema:'agent-control.core-release-receipt/v2',...current,generatedAt:'2026-09-13T00:00:00Z',checks},receiptFile='core-receipt.json',receiptPath=path.join(root,receiptFile),receiptBytes=Buffer.from(`${JSON.stringify(receipt,null,2)}\n`);fs.writeFileSync(receiptPath,receiptBytes,{mode:0o600});
  ...
  return {parent,root,receiptFile,approvalFile,current};
}
