...
export async function qualifyAccountProfile(input: {registry: ModelRegistry; providerId: string; accountProfileId: string; command?: string; cwd?: string; timeoutMs?: number; environment?: NodeJS.ProcessEnv; probe?: (command: string, cwd: string, timeoutMs: number, environment: NodeJS.ProcessEnv) => Promise<CodexChatGptAuth>; nodeExecution?: CodexNodeExecutionPort; version?: string}) {
  ...
  const nodeId = account.nodeId ?? 'controller', startedAt = new Date().toISOString(), version = input.version ?? `account-qualification-${startedAt.slice(0, 10)}`;
  input.registry.setAccountQualification({providerId: input.providerId, accountProfileId: input.accountProfileId, nodeId, state: 'QUALIFYING', version, checkedAt: startedAt, capabilities: [], evidence: []});
  try {
    ...
    const record: AccountProfileQualificationRecord = {providerId: provider.id, accountProfileId: account.id, nodeId, state: 'QUALIFIED', version, checkedAt: completedAt, qualifiedAt: completedAt, capabilities: [...new Set([...(account.capabilities ?? []), 'codex-chatgpt-authenticated'])], evidence: ['codex-login-status:chatgpt', ...discovery]};
    input.registry.setAccountQualification(record);
    return {record, evidence: {schema: 'agent-control.account-profile-qualification/v1', providerId: provider.id, accountProfileId: account.id, nodeId, state: 'QUALIFIED', startedAt, completedAt, checks: [{id: 'codex-chatgpt-auth', passed: true}]} satisfies AccountProfileQualificationEvidence};
  } catch (error) {
    const completedAt = new Date().toISOString(), detail = safe(error);
    const record: AccountProfileQualificationRecord = {providerId: provider.id, accountProfileId: account.id, nodeId, state: 'FAILED', version, checkedAt: completedAt, capabilities: [], evidence: [], detail};
    input.registry.setAccountQualification(record);
    return {record, evidence: {schema: 'agent-control.account-profile-qualification/v1', providerId: provider.id, accountProfileId: account.id, nodeId, state: 'FAILED', startedAt, completedAt, checks: [{id: 'codex-chatgpt-auth', passed: false}], detail} satisfies AccountProfileQualificationEvidence};
  }
}
