...
import {registerOpenVoiceActions} from './openvoice-actions.js';
...
export function buildJobRuntime(config: AgentControlConfig, stateRoot = process.env.AGENT_CONTROL_STATE_DIR || path.resolve('.agent-control'), manifestDir = process.env.AGENT_CONTROL_JOB_DIR || path.resolve('config/jobs')) {
  const actions = registerOpenVoiceActions(registerReferenceActions()), catalog = new JobCatalog(actions.ids()).loadDirectory(manifestDir), workers = WorkerRegistry.fromConfig(config.resources);
