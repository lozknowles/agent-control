...
function fixture() {
  ...
  const workers = new WorkerRegistry().register({id: 'controller', capabilities: ['repository.git'], health: 'healthy', capacity: 1, active: 0, observedAt: '2026-09-07T10:00:00.000Z'});
...
test('real model-backed Job path seals a proposal artifact before semantic governance and independent verification', async () => {
  ...
  const workers = new WorkerRegistry().register({id: 'controller', capabilities: ['repository.git', 'model.execute'], health: 'healthy', capacity: 1, active: 0, observedAt: '2026-09-07T10:00:00.000Z'}), state = path.join(setup.root, 'model-state'), ledger = new RunLedger(path.join(state, 'runs.json')), artifacts = new ArtifactStore(path.join(state, 'artifacts'));
