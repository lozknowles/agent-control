...
export class OpenAICompatibleProviderClient {
  ...
  async invoke(model: ModelConfig, input: ProviderPromptInput, options: {timeoutMs?: number; maximumOutputTokens?: number; structured?: boolean; outputSchema?: Record<string, unknown>; toolProbe?: string; systemPrompt?: string; temperature?: number; signal?: AbortSignal; onTelemetry?: (event: ProviderInvocationTelemetry) => void} = {}): Promise<ModelInvocationResult> {
    ...
    const body = wire === 'chat-completions'
      ? {model: model.providerModel, messages: [...(options.systemPrompt ? [{role: 'system',content:options.systemPrompt}] : []),{role: 'user', content: renderedInput}], max_tokens: options.maximumOutputTokens ?? 256, ...(options.structured ? {temperature: 0, response_format: responseFormat} : options.temperature === undefined ? {} : {temperature:options.temperature}), ...(options.toolProbe ? {tools: [{type: 'function', function: {name: options.toolProbe, description: 'Return the requested qualification marker', parameters}}], tool_choice: {type: 'function', function: {name: options.toolProbe}}} : {})}
      : {model: model.providerModel, input: promptCache.input ?? renderedInput, max_output_tokens: options.maximumOutputTokens ?? 256, ...promptCache.parameters, ...(options.systemPrompt ? {instructions:options.systemPrompt} : {}), ...(options.structured ? {text: {format: options.outputSchema ? {type: 'json_schema', name: 'agent_control_output', strict: true, schema: options.outputSchema} : {type: 'json_object'}}} : options.temperature === undefined ? {} : {temperature:options.temperature}), ...(options.toolProbe ? {tools: [{type: 'function', name: options.toolProbe, description: 'Return the requested qualification marker', parameters, strict: true}], tool_choice: {type: 'function', name: options.toolProbe}} : {})};
    try {
