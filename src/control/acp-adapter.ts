...
interface AcpAdapterSession {acpSessionId: string; governedSessionId: string; actorId: string; cwd: string; parcelIds: string[]; createdAt: string; closed: boolean;}
...
export class AcpAgentControlAdapter {
  ...
  constructor(
    ...
    readonly update: (notification: AcpSessionUpdate) => void = () => undefined,
    readonly clock: () => string = () => new Date().toISOString(),
  ) { this.ensureControlPlaneIdentity(); }

  async handle(request: AcpJsonRpcRequest): Promise<AcpJsonRpcResponse | null> {
    const id = request.id ?? null;
    try {
      switch (request.method) {
        case 'initialize': return ok(id, {protocolVersion: 1, agentCapabilities: {loadSession: true, promptCapabilities: {image: false, audio: false, embeddedContext: true}, sessionCapabilities: {list: {}, resume: {}, close: {}}}, agentInfo: {name: 'Agent Control', version: '3.5.0'}});
        ...
        case 'session/list': return ok(id, {sessions: [...this.sessions.values()].filter(value => !value.closed).map(value => ({sessionId: value.acpSessionId, cwd: value.cwd, updatedAt: value.createdAt, title: `Agent Control ${value.acpSessionId}`}))});
  ...
  private newSession(params: Record<string, unknown>) {
    ...
    this.sessions.set(acpSessionId, {acpSessionId, governedSessionId: session.id, actorId: this.principalActorId, cwd, parcelIds: [], createdAt: this.clock(), closed: false});
  ...
  private async prompt(request: AcpJsonRpcRequest, params: Record<string, unknown>) {
    ...
    this.update({jsonrpc: '2.0', method: 'session/update', params: {sessionId: session.acpSessionId, update: {sessionUpdate: 'agent_message_chunk', content: {type: 'text', text: 'Agent Control accepted the prompt for governed planning.'}}}});
    ...
    this.update({jsonrpc: '2.0', method: 'session/update', params: {sessionId: session.acpSessionId, update: {sessionUpdate: 'tool_call_update', toolCallId: outcome.parcelId, status: outcome.status, title: 'Governed Work Parcel', _meta: {agentControl: {parcelId: outcome.parcelId, runId: outcome.runId, evidenceIds: outcome.evidenceIds ?? []}}}}});
    return {stopReason: outcome.status === 'CANCELLED' ? 'cancelled' : 'end_turn', _meta: {agentControl: {sessionId: session.governedSessionId, parcelId: outcome.parcelId, runId: outcome.runId, status: outcome.status, evidenceIds: outcome.evidenceIds ?? []}}, ...(outcome.result ? {result: outcome.result} : {})};
  }
  ...
  private closeSession(params: Record<string, unknown>) { const session = this.mustSession(params); session.closed = true; this.sessions.set(session.acpSessionId, session); this.identities.updateSession(session.governedSessionId, {status: 'CLOSED'}, session.actorId); }
