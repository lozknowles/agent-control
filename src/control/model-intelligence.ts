...
function classifyEvaluationFailure(error: unknown): EvaluationFailureClass { const value = error instanceof Error ? error.message.toLowerCase() : String(error).toLowerCase(); return /auth|credential|login/.test(value) ? 'AUTHENTICATION_UNAVAILABLE' : /capabilit|unsupported/.test(value) ? 'CAPABILITY_UNAVAILABLE' : /provider|endpoint|connection|network/.test(value) ? 'PROVIDER_UNAVAILABLE' : /timeout|timed out/.test(value) ? 'TIMEOUT' : /policy|denied|approval/.test(value) ? 'POLICY_DENIED' : /architecture|contract|schema/.test(value) ? 'ARCHITECTURE_REGRESSION' : 'UNKNOWN'; }
