...
export class PoeOperatorRuntime {
  ...
  private async queryRegistry(text: string, conversation: PoeConversation, reference?: PoeObjectReference): Promise<PoeEvidenceResult | undefined> {
    ...
    if (/\b(?:systems?|machines?|pixel|tailscale|termux|readiness)\b/i.test(text) && !/facebook|job|how.*works|purpose|explain.*system/i.test(text)) {
  ...
  private propose(id: string, prompt: string, conversation: PoeConversation) {
    ...
    if (!proposal || proposal.state !== 'SUBMITTED' && Date.parse(proposal.expiresAt) <= this.clock().getTime()) {
      const sealed = {id: key, conversationId: conversation.id, actor: conversation.actorId, prompt, job: id, parameters, definitionHash, policyHash, createdAt: this.clock().toISOString(), expiresAt: new Date(this.clock().getTime() + 10 * 60_000).toISOString()};
      proposal = {...sealed, hash: digest(sealed), state: 'WAITING_FOR_APPROVAL'};
      this.proposals.set(key, proposal); this.save();
    }
    return this.result('Review job proposal', proposal.state === 'SUBMITTED' ? `This request already created ${proposal.parcelId}.` : 'Nothing has started. Review the exact job, inputs, worker placement and effects, then use the explicit approval control. The proposal expires after ten minutes. The machinery is practising restraint.', [this.fact('Proposal', proposal, `poe-operation:${proposal.id}`), this.fact('Effects', policy, `operator-registration:${id}`, 'CONFIGURED_CAPABILITY'), this.fact('Live preflight', preflight, `workers:preflight:${id}`)], proposal.parcelId ? [{kind: 'parcel', id: proposal.parcelId}] : []);
  }
  ...
  approve(id: string, hash: string, conversation: PoeConversation) {
    ...
    const origin = governedRequestOrigin({channel: 'poe/dashboard', modality: 'dashboard', receivedAt: proposal.createdAt, authentication: 'dashboard-bearer', actorId: proposal.actor, authority: [`conversation:${conversation.id}`, `operation:${id}`, `approved-sha256:${hash}`], messageReference: id, identityReference: digest(proposal.actor), request: proposal.prompt});
