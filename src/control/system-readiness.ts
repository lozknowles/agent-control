...
export function deriveSystemReadiness(input: {providers?: ProviderRegistry; resources: RegisteredResource[]; managedNodes?: ManagedNodeManager; workers: WorkerRegistration[]; runs: RunRecord[]; invocations: ModelInvocationObservation[]}): SystemReadiness[] {
  ...
  const providers = input.providers?.list().map(provider => {
    const state = input.providers!.health(provider.id), records = input.invocations.filter(item => item.provider === provider.id), active = records.filter(item => item.state === 'RUNNING').length, recent = records.at(-1);
    ...
    if (state?.health === 'offline') { execution = 'OFFLINE'; reachable = 'no'; blockingReason = state.detail ?? 'Provider endpoint is offline'; }
    else if (credentialMissing) { execution = 'AUTH REQUIRED'; reachable = state?.health === 'healthy' ? 'yes' : 'unknown'; blockingReason = 'Provider credentials are required'; }
    else if (!state || ['unknown','unconfigured'].includes(state.health)) { execution = 'UNKNOWN'; reachable = 'unknown'; blockingReason = 'Provider has not been successfully probed'; }
