...
export interface SpeechCacheEntry {schema:'agent-control.mallow-speech-cache-entry/v1';key:string;text:string;voiceIdentity:string;voiceModel:string;voiceVersion:string;provider:string;audioSha256:string;durationMs:number;createdAt:string;chunks:Array<{bytes:string;durationMs:number}>;}
...
export class GovernedSpeechCache implements RealtimeSpeechProvider {
  ...
  async *frames(text:string,signal:AbortSignal){
    ...
    try{const entry=JSON.parse(fs.readFileSync(file,'utf8')) as SpeechCacheEntry;if(entry.key!==key||entry.text!==text||entry.voiceVersion!==this.voice.version)throw Error('speech_cache_identity_mismatch');for(const item of entry.chunks){signal.throwIfAborted();yield{bytes:Buffer.from(item.bytes,'base64'),durationMs:item.durationMs};}return;}catch(error){if((error as NodeJS.ErrnoException).code!=='ENOENT')throw error;}
    ...
    const bytes=Buffer.concat(chunks.map(chunk=>Buffer.from(chunk.bytes))),entry:SpeechCacheEntry={schema:'agent-control.mallow-speech-cache-entry/v1',key,text,voiceIdentity:this.voice.id,voiceModel:this.voice.model,voiceVersion:this.voice.version,provider:this.source.id,audioSha256:createHash('sha256').update(bytes).digest('hex'),durationMs:chunks.reduce((sum,chunk)=>sum+chunk.durationMs,0),createdAt:new Date().toISOString(),chunks:chunks.map(chunk=>({bytes:Buffer.from(chunk.bytes).toString('base64'),durationMs:chunk.durationMs}))};
