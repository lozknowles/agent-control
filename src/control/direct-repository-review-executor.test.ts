...
test('production engine retry preserves one logical Run while opening an immutable token thread for each execution attempt', async t => {
  ...
  const attemptParcels=completed.workParcelIds.map(id=>parcels.get(id)!);assert.ok(attemptParcels.every(parcel=>parcel.stages[0].requestedRoute?.allowFallback===false),'Every attempt must preserve the sealed fallback prohibition');assert.equal(attemptParcels.find(parcel=>parcel.status==='FAILED')?.audit.invocations[0]?.verifierResult,'not-applicable-transport-failure');assert.equal(attemptParcels.find(parcel=>parcel.status==='SUCCEEDED')?.audit.invocations[0]?.verifierResult,'PASS');assert.equal(completed.usage.totalTokens,undefined);assert.equal(completed.usage.unknownUsageInvocations,1);
