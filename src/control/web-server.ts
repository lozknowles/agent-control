...
async function handle(service: AgentControlService, request: IncomingMessage, response: ServerResponse, options: Required<Pick<WebServerOptions, 'host' | 'port' | 'assetsDir'>> & WebServerOptions) {
  ...
  if(method==='POST'&&securityAuditMatch){
    ...
    if(action==='resume'){const audit=options.securityAudits.resume(id,actor),run=service.createJobRun('security-audit',{auditId:id},actor);return json(response,202,{audit:options.securityAudits.linkRun(id,run.id,actor),run});}
  ...
  if(method==='GET'&&url.pathname==='/api/workspaces'){validateOperatorRequest(request,options);return json(response,200,service.workspaces());}
