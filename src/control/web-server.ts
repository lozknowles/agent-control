...
async function handle(service: AgentControlService, request: IncomingMessage, response: ServerResponse, options: Required<Pick<WebServerOptions, 'host' | 'port' | 'assetsDir'>> & WebServerOptions) {
  ...
  if (method === 'POST') {
    ...
    if(templateMatch?.[2]==='use')return json(response,201,service.useAgentTemplate(decodeURIComponent(templateMatch[1]),{version:String(body.version??''),digest:String(body.digest??''),job:String(body.job??''),parameters:body.parameters&&typeof body.parameters==='object'&&!Array.isArray(body.parameters)?body.parameters as Record<string,unknown>:{},permissions:Array.isArray(body.permissions)?body.permissions as Array<{kind:string;scope:string}>:[],prompt:String(body.prompt??''),requestKey:String(body.requestKey??'')},actor));
