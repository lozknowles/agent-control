...
export interface SpeechChunk {bytes:Uint8Array;durationMs:number;}
...
export interface RealtimeInputEvent {type:'speech_started'|'speech_ended'|'transcript_partial'|'transcript_final';text?:string;}
...
export class MallowRealtimePresenceRuntime {
  ...
  async attachInput(id:string,provider:RealtimeInputProvider){const session=this.must(id);if(session.inputController)throw Error('voice_input_already_attached');const controller=new AbortController();session.inputController=controller;try{for await(const event of provider.events(controller.signal)){if(controller.signal.aborted||session.closed)break;if(event.type==='speech_started'){session.state='USER_SPEAKING';this.emit(session,'voice.user_speech.started','USER_SPEAKING',{provider:provider.id});}else if(event.type==='speech_ended')this.emit(session,'voice.user_speech.ended','UNDERSTANDING',{provider:provider.id});else if(event.type==='transcript_partial')await this.partial(id,event.text??'');else if(event.type==='transcript_final')await this.transcript(id,event.text??'');}}catch(error){if(!controller.signal.aborted&&!session.closed){this.emit(session,'voice.input.failed','LISTENING',{provider:provider.id,error:error instanceof Error?error.message:String(error)});this.listen(session);}}finally{if(session.inputController===controller)session.inputController=undefined;}}
  ...
  async interrupt(id:string,reason='user_barge_in'){
    const session=this.must(id),speech=session.speech;if(!speech)return null;const detectedAt=this.time();speech.controller.abort();await this.options.speech.cancel?.(speech.generation);session.speech=undefined;session.state='INTERRUPTED';const latency=this.time()-detectedAt;this.emit(session,'voice.response.interrupted','INTERRUPTED',{reason,interruptionToAudioStopMs:latency,measurement:'runtime output suppression; acoustic device stop not observed'},speech.turnId);this.emit(session,'speech.tts.cancelled','INTERRUPTED',{reason,generation:speech.generation},speech.turnId);this.listen(session);return latency;
  }
  private async speak(session:RuntimeSession,turnId:string,text:string,segment:string,detail:Record<string,unknown>={}){
    ...
    try{for await(const chunk of this.options.speech.frames(text,controller.signal)){if(controller.signal.aborted||session.speech?.generation!==generation)return;if(first===null){first=this.time()-start;this.emit(session,'speech.tts.first_audio',session.state,{segment,textToFirstAudioMs:first,measurement:'first frame accepted by runtime; acoustic output unavailable'},turnId);}await this.options.audioSink?.({sessionId:session.id,turnId,generation,chunk});duration+=chunk.durationMs;this.emit(session,'voice.response.chunk',session.state,{segment,index:chunks++,bytes:chunk.bytes.byteLength,durationMs:chunk.durationMs},turnId);}
      if(controller.signal.aborted||session.speech?.generation!==generation)return;this.emit(session,'speech.tts.completed',session.state,{segment,chunks,audioDurationMs:duration,totalMs:this.time()-start},turnId);session.history.push({role:'assistant',text});
