...
export interface WorkParcelPlanStage {
  ...
  requestedRoute?: {provider?: string; accountProfile?: string; model?: string; modelRole?: string; allowFallback?: boolean; profile?: 'THIN' | 'STANDARD' | 'DEEP'; reason: string};
}
...
export interface WorkParcelStage extends WorkParcelPlanStage {
  ...
  baton?: {schema: 'agent-control.work-parcel-baton/v1'; artifactIds: string[]; outputTypes: string[]} | ParcelBatonView;
...
function normalizeStoredParcel(parcel: WorkParcel) {
  if(parcel.origin){validateVoiceWorkOrigin(parcel.origin);parcel.origin=structuredClone(parcel.origin);}
...
function validateVoiceWorkOrigin(origin:VoiceWorkOrigin){if(origin.schema!=='agent-control.voice-work-origin/v1'||origin.interface!=='realtime-voice'||![origin.voiceSessionId,origin.requestId,origin.at,origin.user,origin.rawStt,origin.effectiveRequest].every(value=>typeof value==='string'&&value.trim())||!Array.isArray(origin.clarifications)||origin.clarifications.some(item=>!item||typeof item.at!=='string'||typeof item.rawStt!=='string')||!origin.audio||!['retained','not-retained'].includes(origin.audio.retention)||(origin.audio.reference!==undefined&&typeof origin.audio.reference!=='string'))throw new Error('voice_work_origin_invalid');return origin;}

export function workParcelExecutionTranscript(parcel:WorkParcel){
  const origin=parcel.origin;
  const first=origin?`ORIGINATING HUMAN REQUEST\nTimestamp: ${origin.at}\nInterface: Realtime Voice\nSession: ${origin.voiceSessionId}\nUser: ${origin.user}\nAudio: ${origin.audio.retention}${origin.audio.reference?` under access-controlled reference ${origin.audio.reference}`:''}\n\nRAW STT:\n${origin.rawStt}\n\nNORMALIZED:\n${origin.normalized??'not performed'}\n\nCLARIFICATION HISTORY:\n${origin.clarifications.length?origin.clarifications.map((item,index)=>`${index+1}. ${item.at} USER / RAW STT\n${item.rawStt}${item.interpretation?`\nInterpretation: ${item.interpretation}`:''}`).join('\n\n'):'none'}\n\nEFFECTIVE REQUEST:\n${origin.effectiveRequest}`:`ORIGINATING HUMAN REQUEST\nUnavailable: this parcel predates typed voice provenance.`;
  const execution=`WORK PARCEL\nID: ${parcel.id}\nStatus: ${parcel.status}\nActor: ${parcel.actor}\nPlanner: ${parcel.planner.kind}\nStages: ${parcel.stages.map(stage=>`${stage.id}=${stage.status}`).join(', ')||'planning'}\nModels: ${parcel.audit.totals.models.join(', ')||'unavailable'}\nTokens: ${parcel.audit.totals.totalTokens??'unavailable'}\nCost: ${parcel.audit.totals.cost??'unavailable'} ${parcel.audit.totals.currency??''}`;
...
export class WorkParcelCoordinator {
  ...
  submitApprovedPlan(prompt: string, actor: string, requestKey: string, input: WorkParcelPlan) {
    ...
    if(existing){if(existing.actor!==actor||existing.prompt!==prompt)throw new Error('parcel_request_identity_mismatch');return existing;}
  ...
  submitApprovedVoicePlan(origin:VoiceWorkOrigin,actor:string,input:WorkParcelPlan,attribution?:WorkAttribution){
    validateVoiceWorkOrigin(origin);const id=`parcel-voice-${createHash('sha256').update(origin.requestId).digest('hex')}`,existing=this.store.get(id);
    if(existing){if(existing.actor!==actor||existing.origin?.requestId!==origin.requestId||existing.prompt!==origin.rawStt)throw new Error('voice_parcel_request_identity_mismatch');return existing;}
    const plan=validateWorkParcelPlan(input,this.runtime);if(plan.objective!==origin.effectiveRequest)throw new Error('voice_parcel_effective_request_mismatch');const at=origin.at,stages=materializeStages(plan.stages),context=contextForPlan(plan,actor,stages,at,origin.effectiveRequest);
    return this.store.add({id,prompt:origin.rawStt,objective:origin.effectiveRequest,actor,...(attribution?{attribution:structuredClone(attribution)}:{}),origin:structuredClone(origin),executionOwner:'work-parcel-coordinator',status:'QUEUED',planner:plan.planner,stages,context,createdAt:at,updatedAt:at,telemetry:emptyTelemetry(),audit:createDecisionAudit(origin.effectiveRequest,plan,this.runtime,at),provenance:[{at,type:'voice.origin',detail:`${origin.voiceSessionId}:${origin.requestId}`},{at,type:'submitted',detail:'Authenticated low-risk voice request accepted as a registered governed Job'}]});
  }
  ...
  accept(prompt: string, actor: string, systems: SystemReadiness[] = [], attribution?: WorkAttribution, origin?: VoiceWorkOrigin) {
    if (!prompt.trim()) throw new Error('work_parcel_prompt_required');
    if(origin){validateVoiceWorkOrigin(origin);if(origin.rawStt!==prompt)throw new Error('voice_work_origin_prompt_mismatch');}
    ...
    const effective=origin?.effectiveRequest??prompt,parcel = this.store.add({id: `parcel-${randomUUID()}`, prompt, objective: effective, actor, ...(attribution ? {attribution: structuredClone(attribution)} : {}), ...(origin?{origin:structuredClone(origin)}:{}), executionOwner: 'work-parcel-coordinator', status: blocked ? 'FAILED' : 'PLANNING', planner: {kind: 'deterministic', reason: blocked ?? 'Planning pending'}, stages: [], context: createParcelContext({goal: effective, actor, at}), createdAt: at, updatedAt: at, ...(blocked ? {endedAt: at} : {}), telemetry: emptyTelemetry(), audit: planningAudit(at, timeline, blocked), provenance: [{at, type: 'submitted', detail: 'Natural-language request durably accepted before planning'}, ...(origin?[{at,type:'voice.origin',detail:`${origin.voiceSessionId}:${origin.requestId}`}]:[]), ...(blocked ? [{at, type: 'readiness.blocked', detail: blocked}] : [])]});
  ...
  async tick() {
    for (const stored of this.store.list().filter(parcel => parcel.executionOwner !== 'direct-repository-review-executor' && ['PLANNING','QUEUED','RUNNING','WAITING'].includes(parcel.status)).reverse()) {
      ...
      if (readyStages.length) {
        for (const ready of readyStages) try {
          ...
          if (routeRequest?.model || routeRequest?.modelRole) {
            ...
            modelRoute = this.models.route({model: routeRequest.model, modelRole: routeRequest.modelRole, accountProfile: routeRequest.accountProfile, nodeId: worker.id, requiredCapabilities: ready.requiredCapabilities, allowFallback: routeRequest.allowFallback});
            ...
            appendParcelContextEvent(context, {type: 'route.selected', stageId: ready.id, summary: `${modelRoute.providerId}/${modelRoute.accountProfileId ?? 'default'}/${modelRoute.modelId}@${modelRoute.providerExecutionNodeId}`, detail: {qualificationVersion: modelRoute.qualificationVersion, fallback: modelRoute.fallback, fallbackReason: modelRoute.fallbackReason, requiredCapabilities: ready.requiredCapabilities}, tags: ['route'], evidence: []});
          }
          const parcelContext = {schema: 'agent-control.run-parcel-context/v1' as const, parcelId: parcel.id, stageId: ready.id, originalGoal: context.active.originalGoal, currentInterpretation: context.active.currentInterpretation, effectiveInstructions: [...context.active.effectiveInstructions], constraints: [...context.active.constraints], successCriteria: context.criteria.map(item => ({id: item.id, description: item.description, status: item.status})), baton};
          const run = this.runtime.createRun(ready.job, ready.parameters, {type: 'manual', actor: `work-parcel:${parcel.id}`, ...(modelRoute ? {modelRoute} : {}), parcelContext}, undefined, parcel.id.startsWith('parcel-social-') ? `${parcel.id}:${ready.id}` : undefined); ready.runId = run.id; ready.status = 'RUNNING'; ready.startedAt = now(); ready.executor = ready.executor ?? run.trigger.modelRoute?.modelId ?? 'agent-control-job-runtime'; parcel.status = 'RUNNING'; parcel.provenance.push({at: ready.startedAt, type: 'stage.started', detail: `${ready.id}:${run.id}`}); appendParcelContextEvent(context, {at: ready.startedAt, type: 'stage.started', stageId: ready.id, summary: `${ready.name} dispatched`, detail: {runId: run.id, job: ready.job, executor: ready.executor}, tags: ['stage'], evidence: [baton.sha256]}); appendAudit(parcel, {at: ready.startedAt, type: 'stage.dispatched', stageId: ready.id, summary: `${ready.name} dispatched`, detail: `Job ${ready.job}; Run ${run.id}; requested route ${routeRequestLabel(ready)}${modelRoute ? `; resolved ${modelRoute.providerId}/${modelRoute.modelId} on ${modelRoute.nodeId}; qualification ${modelRoute.qualificationVersion}` : ''}`});
        }
  ...
  private reconcile(parcel: WorkParcel) {
    ...
    else if (parcel.stages.every(stage => stage.status === 'SUCCEEDED') && allSuccessCriteriaPass(context)) { parcel.status = 'SUCCEEDED'; parcel.endedAt ??= now(); }
