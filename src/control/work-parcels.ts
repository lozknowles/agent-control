...
export interface WorkParcelPlanStage {
  ...
  requestedRoute?: {provider?: string; model?: string; modelRole?: string; allowFallback?: boolean; profile?: 'THIN' | 'STANDARD' | 'DEEP'; reason: string};
}
...
export class WorkParcelCoordinator {
  ...
  async submit(prompt: string, actor: string, attribution?: WorkAttribution) {
    ...
    return this.store.add({id: `parcel-${randomUUID()}`, prompt, objective: plan.objective, actor, ...(attribution ? {attribution: structuredClone(attribution)} : {}), status: 'QUEUED', planner: plan.planner, stages: plan.stages.map(stage => ({...stage, dependsOn: [...(stage.dependsOn ?? [])], parameters: structuredClone(stage.parameters ?? {}), status: 'QUEUED'})), createdAt: at, updatedAt: at, telemetry: emptyTelemetry(), audit: createDecisionAudit(prompt, plan, this.runtime, at), provenance: [{at, type: 'submitted', detail: `Natural-language request accepted; planner=${plan.planner.kind}`} ]});
  }
      ...
      if (ready) {
        try {
          ...
          if (routeRequest?.model || routeRequest?.modelRole) {
            ...
            modelRoute = this.models.route({model: routeRequest.model, modelRole: routeRequest.modelRole, nodeId: worker.id, allowFallback: routeRequest.allowFallback});
            ready.actualRoute = {workers: [worker.id], provider: modelRoute.providerId, model: modelRoute.modelId, profile: routeRequest.profile, reason: modelRoute.fallback ? `Qualified fallback selected: ${modelRoute.fallbackReason}` : `Qualified ${modelRoute.requestedRole ? `role ${modelRoute.requestedRole}` : `model ${modelRoute.modelId}`} selected`};
          }
