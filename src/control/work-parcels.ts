import {instructionHash, canonicalInstructionJson} from './instruction-resolver.js';
import {InstructionManifestStore, baseInstructionSources, instructionIdentity} from './instruction-observer.js';
...
export class WorkParcelStore {
  ...
  constructor(readonly file: string, readonly instructions = new InstructionManifestStore(path.join(path.dirname(file), 'instruction-manifests'))) { if (fs.existsSync(file)) { const parsed = JSON.parse(fs.readFileSync(file, 'utf8')) as Snapshot; if (parsed.version !== 1) throw new Error('unsupported_work_parcel_snapshot'); for (const parcel of parsed.parcels) this.values.set(parcel.id, normalizeStoredParcel(parcel)); } }
  subscribe(listener: (parcel: WorkParcel) => void) { this.listeners.add(listener); return () => this.listeners.delete(listener); }
  add(parcel: WorkParcel) { if (this.values.has(parcel.id)) throw new Error('work_parcel_exists'); this.captureInstructions(parcel); this.values.set(parcel.id, structuredClone(redactSensitiveValue(normalizeStoredParcel(parcel)))); this.save(); const stored = this.get(parcel.id)!; this.notify(stored); return stored; }
  get(id: string): WorkParcel | undefined { const value = this.values.get(id); return value ? {...structuredClone(value), instructionManifestIds: [...new Set([...(value.instructionManifestIds??[]),...this.instructions.list(id).map(item=>item.id)])]} : undefined; }
  list() { return [...this.values.values()].sort((a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt)).map(value => this.get(value.id)!); }
  update(parcel: WorkParcel) { if (!this.values.has(parcel.id)) throw new Error('work_parcel_missing'); this.captureInstructions(parcel); parcel.updatedAt = now(); this.values.set(parcel.id, structuredClone(redactSensitiveValue(normalizeStoredParcel(parcel)))); this.save(); const stored = this.get(parcel.id)!; this.notify(stored); return stored; }
  private captureInstructions(parcel: WorkParcel) {
    try {
      const amendments=parcel.context?.amendments??[];
      const manifest=this.instructions.resolve({identity:instructionIdentity(parcel.id),sources:baseInstructionSources(parcel.prompt,parcel.context?.active.effectiveInstructions??[]),amendments:amendments.map(item=>({id:item.id,hash:instructionHash(canonicalInstructionJson(item)),status:item.status}))});
      parcel.instructionManifestIds=[...new Set([...(parcel.instructionManifestIds??[]),manifest.id])];
    }catch(error){this.instructions.failure(parcel.id,error);}
  }
...
export class WorkParcelCoordinator {
  ...
  constructor(readonly runtime: JobRuntime, readonly store: WorkParcelStore, readonly planner: WorkParcelPlanner, private readonly efficiency?: HarnessEfficiencyLedgerPort, private readonly models?: ModelRegistry, readonly adaptiveOrchestration?: AdaptiveOrchestrationRuntime, readonly cacheExperts?: CacheAwareExpertRuntime) {}
