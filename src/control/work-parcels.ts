...
export interface WorkParcelPlanStage {
  ...
  requestedRoute?: {provider?: string; accountProfile?: string; model?: string; modelRole?: string; allowFallback?: boolean; purpose?: 'EXECUTION' | 'QUALIFICATION'; profile?: 'THIN' | 'STANDARD' | 'DEEP'; reason: string};
}
...
export class WorkParcelCoordinator {
  ...
  submitApprovedPlan(prompt: string, actor: string, requestKey: string, input: WorkParcelPlan) {
    ...
    return this.store.add({id,prompt,objective:plan.objective,actor,executionMode:'LIVE',executionOwner:'work-parcel-coordinator',status:'QUEUED',planner:plan.planner,stages,context,createdAt:at,updatedAt:at,telemetry:emptyTelemetry(),audit:createDecisionAudit(prompt,plan,this.runtime,at),provenance:[{at,type:'submitted',detail:'Approved social template; durable request identity; existing runtime remains authoritative'}]});
  }
      ...
      if (readyStages.length) {
        for (const ready of readyStages) try {
          ...
          if (routeRequest?.model || routeRequest?.modelRole) {
            ...
            modelRoute = this.models.route({model: routeRequest.model, modelRole: routeRequest.modelRole, accountProfile: routeRequest.accountProfile, nodeId: worker.id, requiredCapabilities: ready.requiredCapabilities, allowFallback: routeRequest.allowFallback, purpose: routeRequest.purpose});
