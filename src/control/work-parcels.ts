...
export interface WorkParcelPlanStage {
  ...
  requestedRoute?: {provider?: string; accountProfile?: string; model?: string; modelRole?: string; allowFallback?: boolean; profile?: 'THIN' | 'STANDARD' | 'DEEP'; reason: string; cacheContext?: CacheContextIdentity; cacheScope?: {sessionId?: string; cacheScopeId?: string; backendInstanceId?: string}; cacheScopeByModel?: Record<string, {sessionId?: string; cacheScopeId?: string; backendInstanceId?: string}>};
}
...
export interface WorkParcelStage extends WorkParcelPlanStage {
  ...
  baton?: {schema: 'agent-control.work-parcel-baton/v1'; artifactIds: string[]; outputTypes: string[]; transportContextSha256?: string; transportIntegrityState?: string; cacheExpertDecisionId?: string; cacheExpertId?: string};
  cacheExpertDecisionId?: string;
  actualRoute?: {workers: string[]; provider?: string; accountProfile?: string; accountLabel?: string; accountPlan?: string; accountPlanAuthority?: 'operator-configured' | 'provider-reported'; accountQualification?: string; accountAvailability?: string; model?: string; profile?: string; reason: string};
...
export class WorkParcelCoordinator {
  ...
  async tick() {
    for (const stored of this.store.list().filter(parcel => parcel.executionOwner !== 'direct-repository-review-executor' && ['PLANNING','QUEUED','RUNNING','WAITING'].includes(parcel.status)).reverse()) {
      ...
      if (ready) {
        try {
          const routeRequest = ready.requestedRoute;
          let modelRoute;
          if (routeRequest?.model || routeRequest?.modelRole) {
            ...
            modelRoute = this.models.route({model: selected?.modelId ?? routeRequest.model, modelRole: selected?.modelId ? undefined : routeRequest.modelRole, accountProfile: selected?.accountProfileId ?? routeRequest.accountProfile, nodeId: selected?.nodeId ?? worker.id, requiredCapabilities: required, allowFallback: selected?.modelId ? false : routeRequest.allowFallback});
  ...
  private reconcile(parcel: WorkParcel) {
    for (const stage of parcel.stages) if (stage.runId && !['SUCCEEDED','FAILED','CANCELLED'].includes(stage.status)) {
      ...
      if (terminalRun.has(run.status)) { stage.endedAt = run.endedAt ?? now(); const cacheDecision = stage.cacheExpertDecisionId ? this.cacheExperts?.decision(stage.cacheExpertDecisionId) : undefined; stage.baton = {schema: 'agent-control.work-parcel-baton/v1', artifactIds: [...run.artifacts], outputTypes: run.artifacts.map(id => this.runtime.artifacts.get(id)?.type ?? 'unknown'), ...(stage.cacheExpertDecisionId ? {cacheExpertDecisionId: stage.cacheExpertDecisionId} : {}), ...(cacheDecision?.selectedExpertId ? {cacheExpertId: cacheDecision.selectedExpertId} : {})}; this.recordAdaptiveOutcome(parcel, stage, run); }
    }
