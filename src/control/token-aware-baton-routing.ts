...
export interface AccountRouteIdentity {accountProfileId?: string; accountLabel?: string; accountPlan?: string; accountPlanAuthority?: 'operator-configured' | 'provider-reported'; accountQualification?: string; accountAvailability?: string;}
...
export interface ParcelTokenTotals {
  parcelId: string;
  threads: string[];
  byModel: Array<{providerId: string; accountProfileId: string | null; accountLabel: string | null; accountPlan: string | null; modelId: string; inputTokens: number | null; outputTokens: number | null; totalTokens: number | null; cost: number | null; currency: string | null}>;
...
export interface TokenRoutingDecision {
  ...
  reason: string;
  contextPercent: number | null;
  target?: {providerId: string; accountProfileId?: string; accountLabel?: string; accountPlan?: string; accountPlanAuthority?: 'operator-configured' | 'provider-reported'; accountQualification?: string; accountAvailability?: string; modelId: string};
  batonId?: string;
...
export class TokenAwareBatonRuntime {
  ...
  createBaton(input: BatonInput): VerifiedBaton {
    ...
    if (thread.parcelId !== input.parcelId || thread.providerId !== input.providerId || thread.accountProfileId !== input.accountProfileId || thread.modelId !== input.modelId) throw new Error('token_baton_provenance_mismatch');
  ...
  parcel(parcelId: string): ParcelTokenTotals {
    ...
    for (const thread of threads) {
      ...
      if (!existing) {
        byModel.set(key, {providerId: thread.providerId, accountProfileId: thread.accountProfileId ?? null, accountLabel: thread.accountLabel ?? null, accountPlan: thread.accountPlan ?? null, modelId: thread.modelId, ...clone(thread.latest.cumulative), cost: thread.latest.cost.amount, currency: thread.latest.cost.currency});
        continue;
      }
