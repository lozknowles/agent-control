...
export interface TokenAmounts {inputTokens: number | null; freshInputTokens: number | null; cachedInputTokens: number | null; outputTokens: number | null; totalTokens: number | null;}
...
const emptyAmounts = (): TokenAmounts => ({inputTokens: null, freshInputTokens: null, cachedInputTokens: null, outputTokens: null, totalTokens: null});
...
export class TokenAwareBatonRuntime {
  ...
  parcel(parcelId: string): ParcelTokenTotals {
    ...
    for (const thread of threads) {
      ...
      existing.inputTokens = sum(existing.inputTokens, thread.latest.cumulative.inputTokens); existing.freshInputTokens = sum(existing.freshInputTokens, thread.latest.cumulative.freshInputTokens); existing.cachedInputTokens = sum(existing.cachedInputTokens, thread.latest.cumulative.cachedInputTokens); existing.outputTokens = sum(existing.outputTokens, thread.latest.cumulative.outputTokens); existing.totalTokens = sum(existing.totalTokens, thread.latest.cumulative.totalTokens); existing.cost = sum(existing.cost, thread.latest.cost.amount); existing.currency = existing.currency ?? thread.latest.cost.currency; byModel.set(key, existing);
    }
    const values = [...byModel.values()];
    return {parcelId, threads: threads.map(thread => thread.id).sort(), byModel: clone(values), inputTokens: aggregate(values.map(value => value.inputTokens)), freshInputTokens: aggregate(values.map(value => value.freshInputTokens)), cachedInputTokens: aggregate(values.map(value => value.cachedInputTokens)), outputTokens: aggregate(values.map(value => value.outputTokens)), totalTokens: aggregate(values.map(value => value.totalTokens)), cost: aggregate(values.map(value => value.cost)), currency: [...new Set(values.map(value => value.currency).filter((value): value is string => Boolean(value)))].length === 1 ? values.find(value => value.currency)?.currency ?? null : null};
  }
...
function normalizePersistedAmounts(value: Partial<TokenAmounts> | undefined): TokenAmounts { const normalized: TokenAmounts = {inputTokens: valid(value?.inputTokens), freshInputTokens: valid(value?.freshInputTokens), cachedInputTokens: valid(value?.cachedInputTokens), outputTokens: valid(value?.outputTokens), totalTokens: valid(value?.totalTokens)}; completeInputBreakdown(normalized); if (normalized.totalTokens === null && normalized.inputTokens !== null && normalized.outputTokens !== null) normalized.totalTokens = normalized.inputTokens + normalized.outputTokens; return normalized; }
function completeInputBreakdown(value: TokenAmounts) {
  if (value.inputTokens === null && value.freshInputTokens !== null && value.cachedInputTokens !== null) value.inputTokens = value.freshInputTokens + value.cachedInputTokens;
  if (value.inputTokens !== null && value.cachedInputTokens !== null) { if (value.cachedInputTokens > value.inputTokens) throw new Error('token_telemetry_cached_input_exceeds_total_input'); const derived = value.inputTokens - value.cachedInputTokens; if (value.freshInputTokens === null) value.freshInputTokens = derived; else if (value.freshInputTokens !== derived) throw new Error('token_telemetry_input_breakdown_mismatch'); }
  if (value.inputTokens !== null && value.freshInputTokens !== null && value.cachedInputTokens === null) { if (value.freshInputTokens > value.inputTokens) throw new Error('token_telemetry_fresh_input_exceeds_total_input'); value.cachedInputTokens = value.inputTokens - value.freshInputTokens; }
}
