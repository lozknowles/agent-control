...
import {ensureAndroidProvisioningMission,PIXEL_INSTALL_TERMUX_BOOT_WORK_ID,runAndroidProvisioning} from './android-provisioning-runtime.js';
...
function installReady(){const q=new WorkQueue();ensureAndroidProvisioningMission(q);for(const id of ['detect-adb','install-adb','pairing-approval','qualify-adb','obtain-termux-boot','verify-termux-boot'])q.get(`android.pixel.provision.${id}`)!.status='completed';return q;}
...
test('incomplete Termux Boot install remains durable device review',async()=>{
  ...
  try{const store=new WorkQueueStore(path.join(dir,'queue.json')),q=installReady();await runAndroidProvisioning({queue:q,store,ops:operations(false),maxSteps:1});assert.equal(q.get(PIXEL_INSTALL_TERMUX_BOOT_WORK_ID)?.status,'human-review');assert.equal(q.get(PIXEL_INSTALL_TERMUX_BOOT_WORK_ID)?.resultRef?.startsWith('NEEDS DEVICE INSTALL'),true);}finally{fs.rmSync(dir,{recursive:true,force:true});}
});
