...
export class TargetLlamaRuntime implements TargetTelemetry {
 ...
 producer(c:ActionContext){return {component:'agent-control-runtime',worker:c.worker.id,adapter:this.id,target:this.target.resource.id,environment:this.target.environment,runId:c.run.id,stepId:c.step.id,provenance:'AGENT_CONTROL_RUNTIME_EVIDENCE'};}
