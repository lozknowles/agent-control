...
export interface PoeProjection {
  ...
  voice: {configured: boolean; recognition: boolean; synthesis: boolean; recognitionStatus: 'ready'|'unavailable'|'unknown'; synthesisStatus: 'ready'|'unavailable'|'unknown'; statusObservedAt: string|null; streaming: boolean; bargeIn: boolean; identity: string | null; limitation: string | null};
  observedAt: string;
}
...
export class PoeRuntime {
  ...
  projection(): PoeProjection {
    ...
    const availability=this.options.voiceAvailability?.()??{recognition:this.options.recognition?'unknown' as const:'unavailable' as const,synthesis:this.options.speech?'unknown' as const:'unavailable' as const,observedAt:null};
    return {schema:'agent-control.poe/v1',reasoning:this.options.responseModel?.describe?.()??{state:this.options.responseModel?'CONFIGURED':'UNAVAILABLE',reason:this.options.responseModel?'Route is disclosed on each completed reply.':'No conversational model is configured.'},identity:{...HOST_IDENTITY,modelSelectable:Boolean(this.options.responseModel)},state:active?.state ?? 'IDLE',activeConversationId:active?.id ?? null,conversations:conversations.map(({turns,...item})=>({...clone(item),turnCount:turns.length})),proposals:[...this.proposals.values()].sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt)).map(clone),voice:{configured:Boolean(voice),recognition:Boolean(this.options.recognition),synthesis:Boolean(this.options.speech),recognitionStatus:availability.recognition,synthesisStatus:availability.synthesis,statusObservedAt:availability.observedAt,streaming:this.options.speech?.capabilities().streaming ?? false,bargeIn:true,identity:voice?.id ?? null,limitation:this.options.speech?.capabilities().streaming ? null : 'Provider exposes complete-audio synthesis; first audio is available only after synthesis completes.'},observedAt:this.clock()};
  }
  async ask(input: {voiceReference?:{sessionId:string;delegationId:string};conversationId: string; text: string; channel?: PoeChannel; modality?: 'text'|'voice'; reference?: PoeObjectReference; contentTrust?: PoeTurn['contentTrust']}) {
    ...
    const operatorTurn = this.addTurn(conversation,{...(input.voiceReference?{voiceReference:input.voiceReference}:{}),actor:'operator',channel:conversation.channel,modality:input.modality ?? 'text',text,authority:'OPERATOR',contentTrust:input.contentTrust ?? 'OPERATOR_REQUEST',references:input.reference?[input.reference]:[],evidence:[]});
