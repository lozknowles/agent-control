...
export class PoeRuntime {
  ...
  bargeIn(conversationId:string,actor:string){const conversation=this.mustConversation(conversationId);cleanText(actor,'poe_actor_invalid',192);const controller=this.speechControllers.get(conversationId);if(controller)controller.abort();if(conversation.speaking&&!conversation.speaking.completedAt)conversation.speaking.interruptedAt=this.clock();this.setState(conversation,'LISTENING',{speech:'interrupted',workParcelCancellation:false});this.record(conversation,'interrupted',{turnId:conversation.speaking?.turnId??null,workParcelCancellation:false});return {interrupted:Boolean(controller),workParcelCancelled:false,conversation:clone(conversation)};}
