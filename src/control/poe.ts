...
export class PoeRuntime {
  ...
  bargeIn(conversationId:string,actor:string,playbackTurnId?:string){const conversation=this.mustConversation(conversationId);cleanText(actor,'poe_actor_invalid',192);const controller=this.speechControllers.get(conversationId),playback=playbackTurnId?conversation.speaking?.turnId===cleanText(playbackTurnId,'poe_playback_turn_invalid',192)&&conversation.turns.some(turn=>turn.id===playbackTurnId&&turn.actor==='poe'&&turn.modality==='voice'):false;if(controller)controller.abort();if(conversation.speaking&&(controller||playback))conversation.speaking.interruptedAt=this.clock();this.setState(conversation,'LISTENING',{speech:'interrupted',speechBoundary:controller?'synthesis':playback?'client-playback':'none',workParcelCancellation:false});this.record(conversation,'interrupted',{turnId:conversation.speaking?.turnId??null,speechBoundary:controller?'synthesis':playback?'client-playback':'none',workParcelCancellation:false});return {interrupted:Boolean(controller||playback),workParcelCancelled:false,conversation:clone(conversation)};}
