...
export class JobRuntime {
  ...
  approve(runId: string, policy: string) { const run = this.mustRun(runId); if (!run.approvals.includes(policy)) run.approvals.push(policy); for (const step of run.steps) if (step.status === 'WAITING_FOR_APPROVAL' && step.approval === policy) step.status = 'QUEUED'; return this.ledger.update(run, 'run.approved'); }
