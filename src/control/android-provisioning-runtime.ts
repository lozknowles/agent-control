...
export async function runAndroidProvisioning(o:ProvisioningRunOptions){const pairing=o.queue.get(PIXEL_PAIRING_WORK_ID);if(o.approvePairing&&pairing?.status==='human-review')o.queue.approve(PIXEL_PAIRING_WORK_ID);o.store.save(o.queue);const handler=createAndroidProvisioningHandler(o.ops),events:ExecutionEvent[]=[];for(let i=0;i<(o.maxSteps??100);i++){const adbQualified=await o.ops.detectAdb();const resource=hpubuntuProvisioningResource(adbQualified);const coordinator=new WorkCoordinator(o.queue,undefined,o.store);const executor=new WorkExecutor(coordinator,handler);const event=await executor.step([resource],[{resourceId:resource.id,busy:0,capacity:1}]);events.push(event);o.onEvent?.(event);if(['idle','review','failed','loop'].includes(event.kind))break;}return{events,queue:o.queue};}
export function provisioningSummary(queue:WorkQueue){return queue.all().map(w=>`${w.status.padEnd(12)} ${w.id}${w.resultRef?` ${w.resultRef}`:''}`).join('\n');}
