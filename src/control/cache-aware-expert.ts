...
export class CacheAwareExpertRuntime {
  ...
  observe(input: CacheExpertObservationInput) {
    ...
    const existing = this.snapshot.records.find(item => item.id === id), previousHistory = existing?.taskHistory ?? [], observation = {invocationId: input.invocationId, at: input.completedAt ?? input.observedAt, taskClass: input.taskClass, outcome: input.outcome, verifierResult: input.verifierResult, reusedTokens: reused, processedPromptTokens: processed}, taskHistory = (previousHistory.some(item => item.invocationId === input.invocationId) ? previousHistory.map(item => item.invocationId === input.invocationId ? observation : item) : [...previousHistory, observation]).slice(-100), economics = cacheEconomics(input.pricing, reused, processed);
