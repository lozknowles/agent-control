...
export class CacheAwareExpertRuntime {
  ...
  observe(input: CacheExpertObservationInput) {
    ...
    const id = `expert-${sha256({route: normalizedRoute, context: normalizedContext}).slice(0, 24)}`;
    for (const prior of this.snapshot.records) {
    ...
    const existing = this.snapshot.records.find(item => item.id === id), previousHistory = existing?.taskHistory ?? [], taskHistory = (previousHistory.some(item => item.invocationId === input.invocationId) ? previousHistory : [...previousHistory, {invocationId: input.invocationId, at: input.completedAt ?? input.observedAt, taskClass: input.taskClass, outcome: input.outcome, verifierResult: input.verifierResult, reusedTokens: reused, processedPromptTokens: processed}]).slice(-100), economics = cacheEconomics(input.pricing, reused, processed);
    const record: CacheExpertRecord = {schema: 'agent-control.cache-expert/v1', id, route: normalizedRoute, context: normalizedContext, cache: {authority, source: input.cacheEvidence?.retentionSource ?? input.cacheEvidence?.source ?? 'provider-cache-evidence-unavailable', reusedTokens: reused, processedPromptTokens: processed, cacheWriteTokens: input.cacheEvidence?.cacheWriteTokens ?? null, reuseRatio: finiteRatio(reuseRatio), expectedReuseRatio: finiteRatio(expectedReuseRatio), retainedPromptTokens: retained}, observedAt: existing?.observedAt ?? input.observedAt, lastUsedAt: input.completedAt ?? input.observedAt, taskClass: input.taskClass, capabilities: [...new Set([...(existing?.capabilities ?? []),...input.capabilities])].sort(), outcome: input.outcome, verifierResult: input.verifierResult, health: input.health, evidenceIds: [...new Set([...(existing?.evidenceIds ?? []),input.invocationId, ...(input.evidenceIds ?? [])])].sort(), taskHistory, economics, invalidatedAt: null, invalidationReason: null};
  ...
  assess(input: {parcelId: string; stageId: string; context: CacheContextIdentity; candidates: CacheExpertCandidate[]}) {
    ...
    const decision: CacheExpertDecision = {schema: 'agent-control.cache-expert-decision/v1', id: `cache-decision-${randomUUID()}`, parcelId: input.parcelId, stageId: input.stageId, createdAt: at, context, candidates: assessed, selectedRoute: selected?.route ?? null, selectedExpertId: selected?.expertId ?? null, selectionAuthority: 'cache-score', changedDeclaredRoute: Boolean(selected && declared && routeKey(selected.route) !== routeKey(declared.route)), reason: selected ? selectionReason(selected) : 'No governed eligible route was available; cache affinity made no selection.', verifier: {status: 'PASS', reasons: []}};
