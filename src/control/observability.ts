...
export function projectNodeDashboard(estate:RuntimeMapProjection,id:string,managed:ManagedNodeSnapshot[],work:ReturnType<typeof nodeWorkIndex>) {
  ...
  add(id,'PHYSICAL_CAPACITY','CPU_LOGICAL',node.detail.cpuLogical,'Estate discovery','REPORTED');add(id,'PHYSICAL_CAPACITY','MEMORY_BYTES',node.detail.totalMemoryBytes,'Estate discovery','REPORTED');add(id,'PHYSICAL_CAPACITY','STORAGE_BYTES',node.detail.diskTotalBytes,'Estate discovery','REPORTED');
...
export function projectJobInspector(run:RunRecord,map:RuntimeMapProjection,usage:InspectorUsage,estate:RuntimeMapProjection,nodeIds:string[],operationId?:string,managed:ManagedNodeSnapshot[]=[]){
...
export function inspectorHistory(value:{id:string;title:string;status:string;events:unknown[];operations:unknown[]}){
