...
export class PoeKnowledgeService {
  ...
  enrich(question:string,evidence?:PoeEvidenceResult):PoeEvidenceResult{
    ...
    const categories:[string,RegExp][]=[['usage',/spent|spend|cost|tokens?|cach|retr(?:y|ies)|electricity|energy|work ran locally|successful work per/i],['voice',/\b(?:voice|speech|omnivoice|microphone|whatsapp)\b/i],['regression',/regression|test suite|test progress|tests passed/i],['crew',/crew|who.*member/i],['models',/models?|providers?|reasoning|route|routing|credential/i],['lanes',/lanes?|machines?|systems?|available/i],['work',/running|waiting|blocked|completed|recent|parent|child|verification/i],['handoffs',/baton|handoff|hand.over|continuation/i]];
    for(const [category,pattern] of categories)if(pattern.test(question)){try{const value=redactSensitiveValue(this.options.live(category,question));assertNoSensitiveMaterial(JSON.stringify(value),'poe_knowledge_credentials_forbidden');const snapshotHash=hash(value);facts.push(fact(category==='voice'?'Configured Morrow voice':`Live ${category}`,value,[`runtime:${category}:sha256:${snapshotHash}`],category==='voice'?'CONFIGURED_CAPABILITY':'LIVE_OBSERVED'));}catch{facts.push(fact(`Live ${category}`,'Unavailable',[`runtime:${category}`],'UNAVAILABLE'));}}
