...
test('authenticated greeting is brief and idempotent per conversation and keeps spoken provenance',async()=>{
 const f=setup(),first=f.poe.greeting(f.conversation.id,'web-operator'),second=f.poe.greeting(f.conversation.id,'web-operator');assert.equal(first.turn.id,second.turn.id);assert.equal(first.turn.text,'Good day. I’m POE. How may I help you?');assert.equal(f.poe.conversation(f.conversation.id).turns.length,1);assert.throws(()=>f.poe.greeting(f.conversation.id,'other'),/actor_mismatch/);await f.poe.speechForTurn(f.conversation.id,first.turn.id,'web-operator');assert.match(f.poe.transcript(f.conversation.id),/Voice: original-test-voice/);const next=f.poe.createConversation({actorId:'web-operator',channel:'dashboard'});assert.notEqual(f.poe.greeting(next.id,'web-operator').turn.id,first.turn.id);
});
