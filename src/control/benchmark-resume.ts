...
export function benchmarkResumeCursor(run:RunRecord,spec:LabQualificationSpec,configuration:unknown,store:ArtifactStore,inspectionOnly=false){
 ...
 for(const meta of records.filter(a=>a.name==='lab-qualification-attempt')){
  ...
  if(a.status!=='SUCCEEDED'){if(!inspectionOnly)throw Error('benchmark_resume_execution_ambiguous');unresolved.push(meta.id);continue;}
 ...
 if(records.filter(a=>a.name==='runtime-invocation-dispatch').length!==refs.length+unresolved.length)throw Error('benchmark_resume_uncommitted_dispatch');
