...
export function benchmarkResumeCursor(run:RunRecord,spec:LabQualificationSpec,configuration:unknown,store:ArtifactStore){
 ...
 const refs:PriorLabEvidence[]=[],refusals:string[]=[],responses=new Set<string>(),completed=new Set<string>();
 for(const meta of records.filter(a=>a.name==='lab-qualification-attempt')){
  ...
  if(a.status!=='SUCCEEDED'||completed.has(slot))throw Error('benchmark_resume_execution_ambiguous');
 ...
 if(records.filter(a=>a.name==='runtime-invocation-dispatch').length!==refs.length)throw Error('benchmark_resume_uncommitted_dispatch');
 ...
 return {schema:'agent-control.benchmark-resume-cursor/v1',runId:run.id,specSha256:digest,reused,refusals,outstanding,completedCount:reused.length,plannedCount:spec.cases.length*spec.repetitions,complete:outstanding.length===0};
}
