...
startAndroidNodeMonitoring(jobRuntime, snapshot => service.events.emit('resource.node_changed', {resourceId: snapshot.resourceId, peerId: snapshot.peerId, state: snapshot.state, health: snapshot.health, route: snapshot.route}, undefined, 'android-node-monitor'));
