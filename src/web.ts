...
if(localBenchmark){localBenchmark.admit(localBenchmarkSettings.template);jobRuntime.workers.register({id:'local-benchmark-controller',capabilities:['benchmark.control'],health:'healthy',capacity:1,active:0,observedAt:new Date().toISOString()});service.configureProjection({localBenchmark});}
...
jobRuntime.workers.register({id:'model-intelligence-controller',capabilities:['model.intelligence.control'],health:'healthy',capacity:1,active:0,observedAt:new Date().toISOString()});
