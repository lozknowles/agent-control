...
export function appendEvent(type: string, payload: unknown) { ensureDir(ROOT); fs.appendFileSync(path.join(ROOT, 'events.jsonl'), `${JSON.stringify({at: new Date().toISOString(), type, payload})}\n`); }
