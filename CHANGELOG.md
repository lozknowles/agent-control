...
- Adds a controlled alternating baseline/candidate cache-boundary qualification command. The available Codex CLI cannot exercise explicit Responses cache controls or expose cache-write/billed-cost/current-context telemetry, so measurements remain observational and no economic benefit is claimed without evidence.
- Physically qualifies the real dashboard with concurrent owned work, a genuinely queued Run, mid-run browser reload/SSE reconstruction, a live Codex repository review with fresh/cached token accounting, cancellation and confirmed Linux process-group cleanup. Android pairing/reconnect and Windows physical cancellation remain release-candidate limitations rather than inferred passes.
