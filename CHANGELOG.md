...
- Adds a provider-neutral dynamic catalogue with authenticated model discovery, authority-labelled metadata, bounded smoke checks, frozen benchmark queue integration, historical economics, explicit operator routing admission and automatic eligibility withdrawal after model-intelligence degradation or disappearance from the latest successful catalogue.
...
- Adds a versioned smoke invocation profile for provider-supported reasoning controls, while keeping ordinary execution and frozen benchmarks unchanged. Truncated responses retain normalized partial usage, finish reason and response hash, coding no longer incorrectly requests the structured-output schema, and the exact v2 probe contract is content-hashed.
- Extends the Models dashboard and API with provider endpoint/credential/discovery state, observed request/token limits and quota, model metadata, smoke/benchmark evidence, quality/coding/tool/reliability/latency/token-efficiency trends, and authenticated Discover, Smoke, Queue/Re-benchmark and Enable/Disable controls.
...
- The Nemotron batch passed 9/9 provider-executed tasks but was `PARTIAL` overall because 42/51 capability-gated attempts were unavailable. No NVIDIA model is qualified or routing eligible; provider price, quota, rate limit, TTFT, context occupancy and most catalogue metadata remain unavailable, so no provider-economics claim is made. Full-catalogue benchmarking remains separately gated.
